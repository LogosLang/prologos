...
;; function (merge-hasheq-replace) will union it with existing content.
...
(define (init-macros-cells! prn-box)
  (when prn-box
    ...
    (define-values (net1 sr-cid) (net-new-cell net0 (current-schema-registry) merge-hasheq-replace))
    (current-schema-registry-cell-id sr-cid)
    (define-values (net2 cr-cid) (net-new-cell net1 (current-ctor-registry) merge-hasheq-replace))
    (current-ctor-registry-cell-id cr-cid)
    (define-values (net3 tm-cid) (net-new-cell net2 (current-type-meta) merge-hasheq-replace))
    (current-type-meta-cell-id tm-cid)
    ;; subtype/coercion use hash (equal?-based keys), but merge-hasheq-replace
    ;; works correctly — hash-set preserves the hash type of the accumulator.
    (define-values (net4 st-cid) (net-new-cell net3 (current-subtype-registry) merge-hasheq-replace))
    (current-subtype-registry-cell-id st-cid)
    (define-values (net5 co-cid) (net-new-cell net4 (current-coercion-registry) merge-hasheq-replace))
    (current-coercion-registry-cell-id co-cid)
    (define-values (net6 cap-cid) (net-new-cell net5 (current-capability-registry) merge-hasheq-replace))
    (current-capability-registry-cell-id cap-cid)
    (define-values (net7 ps-cid) (net-new-cell net6 (current-property-store) merge-hasheq-replace))
    (current-property-store-cell-id ps-cid)
    (define-values (net8 fs-cid) (net-new-cell net7 (current-functor-store) merge-hasheq-replace))
    (current-functor-store-cell-id fs-cid)
    ;; Trait + instance registry cells
    (define-values (net9 tr-cid) (net-new-cell net8 (current-trait-registry) merge-hasheq-replace))
    (current-trait-registry-cell-id tr-cid)
    (define-values (net10 tl-cid) (net-new-cell net9 (current-trait-laws) merge-hasheq-replace))
    (current-trait-laws-cell-id tl-cid)
    (define-values (net11 ir-cid) (net-new-cell net10 (current-impl-registry) merge-hasheq-replace))
    (current-impl-registry-cell-id ir-cid)
    (define-values (net12 pir-cid) (net-new-cell net11 (current-param-impl-registry) merge-hasheq-replace))
    (current-param-impl-registry-cell-id pir-cid)
    (define-values (net13 br-cid) (net-new-cell net12 (current-bundle-registry) merge-hasheq-replace))
    (current-bundle-registry-cell-id br-cid)
    ;; specialization-registry uses hash (equal?-based keys: cons pairs)
    (define-values (net14 spr-cid) (net-new-cell net13 (current-specialization-registry) merge-hasheq-replace))
    (current-specialization-registry-cell-id spr-cid)
    (define-values (net15 sel-cid) (net-new-cell net14 (current-selection-registry) merge-hasheq-replace))
    (current-selection-registry-cell-id sel-cid)
    (define-values (net16 sess-cid) (net-new-cell net15 (current-session-registry) merge-hasheq-replace))
    (current-session-registry-cell-id sess-cid)
    ;; Remaining registries
    (define-values (net17 pp-cid) (net-new-cell net16 (current-preparse-registry) merge-hasheq-replace))
    (current-preparse-registry-cell-id pp-cid)
    (define-values (net18 ss-cid) (net-new-cell net17 (current-spec-store) merge-hasheq-replace))
    ...
    (define-values (net20 strat-cid) (net-new-cell net19 (current-strategy-registry) merge-hasheq-replace))
    (current-strategy-registry-cell-id strat-cid)
    (define-values (net21 proc-cid) (net-new-cell net20 (current-process-registry) merge-hasheq-replace))
    (current-process-registry-cell-id proc-cid)
    (define-values (net22 pg-cid) (net-new-cell net21 (current-user-precedence-groups) merge-hasheq-replace))
    (current-user-precedence-groups-cell-id pg-cid)
    (define-values (net23 op-cid) (net-new-cell net22 (current-user-operators) merge-hasheq-replace))
    (current-user-operators-cell-id op-cid)
    (define-values (net24 mr-cid) (net-new-cell net23 (current-macro-registry) merge-hasheq-replace))
...
(define (access-sentinel? x)
  ;; ⚠ MERGE RESOLUTION (slice 7 x P4e-0, 2026-08-09) — BOTH sides, because
  ;; they are not alternatives. P4e-0 added `star-step-with-base?`; slice 7
  ;; replaced the flat disjunction with a lookup over reader-forms.rkt's head
  ;; sets so parse-reader.rkt can consume the SAME list at reader time.
  ;;
  ;; `$star-step` CANNOT become a head-set entry, and that is the subtle part:
  ;; its membership is CONDITIONAL and recursive — a star is base-needing only
  ;; when its PAYLOAD is (`m:0*` yes, `[f 1]*` no). A flat head list cannot say
  ;; "member iff its payload is a member", and adding `$star-step` to one
  ;; unconditionally would re-introduce the measured P4e-0 bug it exists to
  ;; prevent: `def q := [f 1]*` folding the star onto the BINDING MARKER.
  ;; So the head-set lookup covers the flat families and the conditional clause
  ;; rides alongside it, verbatim.
  (or (and (list? x) (pair? x)
           (let ([h (car x)])
             (or (and (= (length x) 2) (memq h arity2-access-sentinel-heads) #t)
                 (and (memq h brace-access-sentinel-heads) #t))))
      (star-step-with-base? x)))
