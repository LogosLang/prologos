...
;;;
;;; *** OCapN compatibility target — Tier A (type-level only) ***
;;; Brought in from PR #28 (LogosLang/prologos branch
;;; claude/ocapn-prologos-implementation-auLxZ) on 2026-05-04 as a
;;; smoke probe for the current branch's elaboration boundary. refr.prologos
;;; declares `capability` + `subtype` only — no value-level reduction —
;;; so it should elaborate cleanly today even though the rest of the
;;; OCapN port (Tier B, gated on Phase 10b) is skipped.
