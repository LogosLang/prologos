...
(test-case "serialize-network-topology: empty network"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 15)  ;; PPN 4C 2A.0: 15 well-known cells (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel, 12=fuel-budget, 13=retraction-stratum, 14=resolution-stratum)
...
(test-case "serialize-network-topology: network with cells and propagator"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 17)  ;; PPN 4C 2A.0: 15 well-known + 2 user (cid-a, cid-b)
