...
(test-case "serialize-network-topology: empty network"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 4)  ;; PAR Track 1: request cell at id 0, BSP-LE Track 2: worldview cache at id 1, R1: rel-store at 2, config at 3
...
(test-case "serialize-network-topology: network with cells and propagator"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 6)  ;; PAR Track 1: +1 request cell, BSP-LE Track 2: +1 worldview cache, R1: +2 well-known cells
