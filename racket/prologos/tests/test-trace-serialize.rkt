...
(test-case "serialize-network-topology: empty network"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 23)  ;; PReduce Track 8 Phase 2: 23 well-known cells (0-21 as before + 22=dispatch-request)
...
(test-case "serialize-network-topology: network with cells and propagator"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 25)  ;; PReduce Track 8 Phase 2: 23 well-known + 2 user (cid-a, cid-b)
