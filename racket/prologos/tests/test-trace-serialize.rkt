...
(test-case "serialize-network-topology: empty network"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 17)  ;; PPN 4C 3A.0: 17 well-known cells (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel, 12=fuel-budget, 13=retraction-stratum, 14=resolution-stratum, 15=fork-on-union-request, 16=fork-contradiction-request)
...
(test-case "serialize-network-topology: network with cells and propagator"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 19)  ;; PPN 4C 3A.0: 17 well-known + 2 user (cid-a, cid-b)
