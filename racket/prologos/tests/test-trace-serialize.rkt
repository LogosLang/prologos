...
(test-case "serialize-network-topology: empty network"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 13)  ;; 1B-iv: 13 well-known cells (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel-cell, 12=fuel-budget-cell)
...
(test-case "serialize-network-topology: network with cells and propagator"
  ...
  (check-equal? (hash-ref (hash-ref js 'stats) 'totalCells) 15)  ;; 1B-iv: 13 well-known + 2 user (cid-a, cid-b)
