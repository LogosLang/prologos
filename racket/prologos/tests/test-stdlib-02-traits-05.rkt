...
(test-case "elem/empty-list"
  ;; Any element not in []
  (check-equal?
   (last (run-ns "(ns le3)\n(imports [prologos::data::list :refer [List nil elem]])\n(imports [prologos::core::eq :refer [nat-eq]])\n(eval (elem Nat nat-eq zero (nil Nat)))"))
   "false : Bool"))
