...
(test-case "auto-implicit/compose-three-vars"
  ...
  (check-equal?
   (run-last (string-append
     "(ns ai4)\n"
     "(defn my-inc [n <Nat>] <Nat> (inc n))\n"
     "(defn compose [g <(-> B C)> f <(-> A B)> x <A>] <C> (g (f x)))\n"
     "(eval (compose my-inc my-inc zero))"))
   "2 : Nat"))
...
(test-case "auto-implicit/no-free-vars"
  ...
  (check-equal?
   (run-last "(ns ai9)\n(defn add [x <Nat> y <Nat>] <Nat> (match y (zero -> x) (inc k -> (inc (add x k)))))\n(eval (add (inc zero) (inc (inc zero))))")
   "3 : Nat"))
...
(test-case "auto-implicit/data-type-name-excluded"
  ...
  (check-equal?
   (run-last (string-append
     ...
     "  (head default xs))\n"
     "(eval (my-head zero (cons (inc zero) nil)))"))
   "1 : Nat"))
