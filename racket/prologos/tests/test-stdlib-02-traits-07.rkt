...
(test-case "implicit/some-zero"
  (check-equal?
   (last (run-ns "(ns imp7)\n(imports [prologos::data::option :refer [Option some unwrap-or]])\n(eval (unwrap-or Nat (suc zero) (some Nat zero)))"))
   "0N : Nat"))
...
(test-case "implicit/bare-none"
  (check-equal?
   (last (run-ns "(ns imp8)\n(imports [prologos::data::option :refer [Option none unwrap-or]])\n(eval (unwrap-or Nat (suc zero) none))"))
   "1N : Nat"))
