...
(printf "- Native ns = total ns - callback ns. After the #:native-op hint fix,~n")
(printf "  W5 (int-arith) routes to the kernel's built-in fire-fns at tags 0-7~n")
(printf "  (KERNEL-INT-ADD-TAG etc.); other workloads' fire-fns are still~n")
(printf "  Racket callbacks because they have no kernel-native equivalent.~n")
(printf "- Phase 7 migration adds native dispatch for more fire-fns (e.g.~n")
(printf "  expr-suc, expr-reduce arm-match) by adding kernel-side implementations~n")
(printf "  + entries to NATIVE-OP-TAGS in preduce-backend-hybrid.rkt.~n")
