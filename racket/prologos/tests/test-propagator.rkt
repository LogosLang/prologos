...
(test-case "make-prop-network: creates empty network"
  ...
  (check-equal? (prop-network-next-cell-id net) 15)  ;; well-known: 0=decomp-request, 1=worldview-cache, 2=relation-store, 3=config, 4=naf-pending, 5=pool-config, 6-9=topology subsystem cells (A1), 10=classify-inhabit-request (PPN 4C Phase 3c-iii), 11=fuel, 12=fuel-budget (Tropical Addendum), 13=retraction-stratum-request, 14=resolution-stratum-request (PPN 4C 2A.0)
...
(test-case "net-new-cell: returns new network and cell-id"
  ...
  (check-equal? (cell-id-n cid) 15))  ;; PPN 4C 2A.0: first user cell is 15 (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel, 12=fuel-budget per 1B-iv, 13=retraction-stratum-request, 14=resolution-stratum-request per 2A.0)
...
(test-case "net-new-cell: sequential cell ids"
  ...
  (check-equal? (cell-id-n cid1) 15)  ;; PPN 4C 2A.0: offset by 15 (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel-cell, 12=fuel-budget-cell, 13=retraction-stratum, 14=resolution-stratum)
  (check-equal? (cell-id-n cid2) 16)
  (check-equal? (cell-id-n cid3) 17))
...
(test-case "net-new-cells-batch: creates N cells with contiguous IDs"
  ...
  (check-equal? (map cell-id-n ids) '(15 16 17))  ;; PPN 4C 2A.0: offset by 15 (post 13/14 stratum-request cells)
  ;; All cells readable with initial values
  (for ([cid (in-list ids)])
    (check-equal? (net-cell-read net* cid) 'bot))
  ;; next-cell-id advanced by 3 from 15 → 18
  (check-equal? (prop-network-next-cell-id net*) 18))
...
(test-case "net-new-cells-batch: IDs continue from existing cells"
  ...
  ;; PPN 4C 2A.0: existing cell is ID 15 (post 13/14 stratum-request cells); batch starts at 16
  (check-equal? (cell-id-n existing-id) 15)
  (check-equal? (map cell-id-n ids) '(16 17))
  (check-equal? (prop-network-next-cell-id net2) 18))
