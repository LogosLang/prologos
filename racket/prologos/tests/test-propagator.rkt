...
(test-case "make-prop-network: creates empty network"
  ...
  (check-equal? (prop-network-next-cell-id net) 5)  ;; cell-id 0=decomp-request, 1=worldview-cache, 2=relation-store, 3=config, 4=naf-pending
...
(test-case "net-new-cell: returns new network and cell-id"
  ...
  (check-equal? (cell-id-n cid) 5))  ;; BSP-LE Track 2B R1: first user cell is 5 (0=decomp, 1=worldview, 2=rel-store, 3=config, 4=naf-pending)
...
(test-case "net-new-cell: sequential cell ids"
  ...
  (check-equal? (cell-id-n cid1) 5)  ;; BSP-LE Track 2B: offset by 5 (0=decomp, 1=worldview, 2=rel-store, 3=config, 4=naf-pending)
  (check-equal? (cell-id-n cid2) 6)
  (check-equal? (cell-id-n cid3) 7))
...
(test-case "net-new-cells-batch: creates N cells with contiguous IDs"
  ...
  (check-equal? (map cell-id-n ids) '(5 6 7))  ;; BSP-LE Track 2B: offset by 5 (0=decomp, 1=worldview, 2=rel-store, 3=config, 4=naf-pending)
  ;; All cells readable with initial values
  (for ([cid (in-list ids)])
    (check-equal? (net-cell-read net* cid) 'bot))
  ;; next-cell-id advanced by 3 from 5
  (check-equal? (prop-network-next-cell-id net*) 8))
...
(test-case "net-new-cells-batch: IDs continue from existing cells"
  ...
  ;; BSP-LE Track 2B: existing cell is ID 5 (0=decomp, 1=worldview, 2=rel-store, 3=config, 4=naf-pending), batch starts at 6
  (check-equal? (cell-id-n existing-id) 5)
  (check-equal? (map cell-id-n ids) '(6 7))
  (check-equal? (prop-network-next-cell-id net2) 8))
