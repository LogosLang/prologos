...
(test-case "make-prop-network: creates empty network"
  ...
  (check-equal? (prop-network-next-cell-id net) 13)  ;; well-known: 0=decomp-request, 1=worldview-cache, 2=relation-store, 3=config, 4=naf-pending, 5=pool-config, 6-9=topology subsystem cells (A1), 10=classify-inhabit-request (PPN 4C Phase 3c-iii)
...
(test-case "net-new-cell: returns new network and cell-id"
  ...
  (check-equal? (cell-id-n cid) 13))  ;; PPN 4C 3c-iii: first user cell is 13 (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel, 12=fuel-budget per 1B-iv) (0-5 well-known, 6-9 topology, 10 classify-inhabit-request)
...
(test-case "net-new-cell: sequential cell ids"
  ...
  (check-equal? (cell-id-n cid1) 13)  ;; 1B-iv: offset by 13 (0-5 well-known, 6-9 topology, 10 classify-inhabit, 11=fuel-cell, 12=fuel-budget-cell)
  (check-equal? (cell-id-n cid2) 14)
  (check-equal? (cell-id-n cid3) 15))
...
(test-case "net-new-cells-batch: creates N cells with contiguous IDs"
  ...
  (check-equal? (map cell-id-n ids) '(13 14 15))  ;; PPN 4C 3c-iii: offset by 13 (0-5 well-known, 6-9 topology, 10 classify-inhabit-request)
  ;; All cells readable with initial values
  (for ([cid (in-list ids)])
    (check-equal? (net-cell-read net* cid) 'bot))
  ;; next-cell-id advanced by 3 from 13 → 16
  (check-equal? (prop-network-next-cell-id net*) 16))
...
(test-case "net-new-cells-batch: IDs continue from existing cells"
  ...
  ;; PPN 4C 3c-iii: existing cell is ID 13 (0-5 well-known, 6-9 topology, 10 classify-inhabit-request), batch starts at 12
  (check-equal? (cell-id-n existing-id) 13)
  (check-equal? (map cell-id-n ids) '(14 15))
  (check-equal? (prop-network-next-cell-id net2) 16))
