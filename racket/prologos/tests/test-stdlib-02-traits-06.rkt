...
(test-case "native-ctor/list-type-0"
  ...
  (check-equal?
   (last (run-ns "(ns nc1)\n(imports [prologos::data::list :refer [List]])\n(check (List Nat) : (Type 0))"))
   "OK"))
...
(test-case "native-ctor/option-type-0"
  ;; Option Nat : Type 0
  (check-equal?
   (last (run-ns "(ns nc2)\n(imports [prologos::data::option :refer [Option]])\n(check (Option Nat) : (Type 0))"))
   "OK"))
