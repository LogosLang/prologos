...
(cond
  ...
  [else
   ...
   ;; Link both objects.
   (define hamt-obj
     (or (getenv "PROLOGOS_HAMT_OBJ") "runtime/prologos-hamt.o"))
   (printf "Linking ~a + ~a + ~a -> ~a~n" ll-path runtime-obj hamt-obj (out-path))
   (define link-ok?
     (system* (find-executable-path clang)
              ll-path runtime-obj hamt-obj "-o" (out-path)))
   (unless link-ok?
