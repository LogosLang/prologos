...
- **✅ TWO OF THE THREE MISLEADING RUNNER BEHAVIOURS ARE FIXED (2026-08-13, CIU T6)** — this entry described them as permanent warts to work around; they were bugs, and both are repaired. Kept here with their old symptoms because those symptoms are what a stale log or an older branch will still show.
  1. **The per-file timeout is now REAL and matches what is printed.** Was: `--all` killed at a hardcoded **120s** while the banner announced `timeout: 600s`, because `run-affected-tests.rkt` spawned `batch-worker.rkt` with **no `--file-timeout`** — so the documented knob controlled nothing and the true limit was invisible. Now threaded, and the runner default is **300s**. `--timeout N` genuinely kills at N (verified: `--timeout 20` on a 88s file → `TIMEOUT (20.0s)`). ⚠ The old failure signature is still worth knowing: `wall_ms: 120003` with `tests: N (0 failures)` is a TIMEOUT, not an assertion failure — a timed-out file reports **zero failures**, so a count-based check reads it as green.
  2. **"⛔ DEAD WORKERS" now means the workers are dead.** Was: a 30s no-output heuristic that fired on any legitimately slow FIRST file and advised recompiling — which is why `--tests tests/test-path-selection.rkt` "aborted" on a clean tree after a clean compile, and why sessions worked around it with a bare `racket -e '(require (file "…"))'`. The 30s window is now a **liveness probe that re-arms** while `subprocess-status` says the workers are running, bounded by the real per-file timeout. That workaround is no longer needed (verified: `[1/1] test-path-selection.rkt PASS (88.6s)`).
  3. **Still true, and unchanged**: before blaming a change for a timing failure, **re-run the same file on the untouched base worktree** — that A/B settles it in one command. Check `uptime` load first: other worktrees/sessions will starve a run to 0% CPU.
  ⚠ **Headroom is not permanent.** `test-path-selection.rkt` measured 112–122s across runs on the same commit — it has already crossed the old 120s limit — so a growing test file can re-enter this failure mode. Watch the slowest file against the limit, not against your memory of it.
