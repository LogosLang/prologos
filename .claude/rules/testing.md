...
- **`--no-precompile` + `raco make driver.rkt` TRAP** -- NEVER combine manual `raco make driver.rkt` with `--no-precompile` on the suite runner. `raco make driver.rkt` recompiles production code but NOT test files. `--no-precompile` bypasses the runner's `precompile-modules!` which DOES compile test files. The combination leaves test `.zo` stale. The batch worker uses `dynamic-require` which trusts cached `.zo` — **stale test `.zo` produces silently wrong results, NOT linklet errors**. If you need to pre-compile for timing isolation, run `raco make tests/*.rkt` as well, or let the runner handle it without `--no-precompile`. Confirmed trap in BSP-LE Track 2B Phase T-a diagnosis (multiple cycles of "passes individually, fails in batch"). See `tools/run-affected-tests.rkt` stale-.zo detection for the warning that catches this.
