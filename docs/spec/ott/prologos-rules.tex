...
\begin{itemize}
  \item $\mathtt{m0}$: erased (compile-time only, 0 runtime uses)
  \item $\mathtt{m1}$: linear (exactly 1 runtime use)
  \item $\mathtt{mw}$: unrestricted (any number of uses)
\end{itemize}
...
\section{Multiplicity Operations}

\ottdefnsmultiplicityXops
...
\subsection{Universe Level Inference}
...
\begin{itemize}
  \item \textbf{QTT Inference}: $\Gamma \vdash e \Rightarrow A ; \mathcal{U}$
  \item \textbf{QTT Checking}: $\Gamma \vdash e \Leftarrow A ; \mathcal{U}$
\end{itemize}

Usage contexts compose via pointwise semiring operations:
$\mathcal{U}_1 \oplus \mathcal{U}_2$ (addition) and
$m \otimes \mathcal{U}$ (scalar multiplication).
...
\subsection{QTT Top-Level Check}
