...
> **SUPERSEDED 2026-06-10 by the Track 0.1 SM2 lock ([D.1 §2](../tracking/2026-06-10_PREDUCE_TRACK01_DESIGN.md))**: cell value = componentwise-ACI product `{best | alts | canonical | provenance}`; merge IS the order (`:enrichment :semilattice [:Q-module Q]`, Q-polymorphic per owner D7) — the refinement-poset declaration below had no substrate realization. The "cell-id = structural hash of canonical representative" scheme below is circular once classes merge; resolved by the D3 three-key separation. Adhesive inheritance routes through shared semilattice enrichment (TBGH), not Biondo adhesivity. Congruence = S0 signature-set watchers. This section is retained as the historical Stage-1 lean.
