...
#+begin_example
check-confluence(clauses):
  1. For each pair of clauses (c1, c2) (including c1 with itself):
...
*** 2.1.5 Scope: Module-Level vs. Whole-Program
...
| Per-module | Medium (all calls in module) | O(n^2) per module | Phase 3a full |
| Whole-program | High (all calls) | O(n^3) worst case | Phase 3a+ (optional) |

For Phase 3a, module-level analysis is the right target. Whole-program
analysis can be deferred or made lazy (analyze on demand when narrowing
encounters an unknown HO argument).
...
Implementation via a cost-bound cell:

#+begin_src racket
;; Branch-and-bound as propagator
(define (install-bb-min net cost-cell)
  ;; Cost bound cell: starts at +inf, decreases monotonically
  (define-values (net* bound-cell)
    (net-new-cell net +inf.0
      (lambda (old new) (min old new))  ;; merge = min
      (lambda (v) #f)))                 ;; never contradicts
  ;; Propagator: cost-cell.lo > bound-cell → contradiction
  (define-values (net** _pid)
    (net-add-propagator net* (list cost-cell bound-cell) '()
      (lambda (net)
        (define cost (net-cell-read net cost-cell))
        (define bound (net-cell-read net bound-cell))
...
1. *Interval domain for non-numeric types*: Can we define abstract domains
   for List (length interval), String (length interval), or algebraic types
   in general? Length-based abstraction is a natural Galois connection.
...
** DD-3: Module-Level 0-CFA

Scope 0-CFA to module level for Phase 3a. This balances precision (captures
all intra-module function flows) against cost (avoids whole-program
analysis). Cross-module flows can be handled conservatively (treat imported
functions as unknown flow sets) or eagerly (analyze transitively).
...
They run once per =defn= and do not affect runtime performance. 0-CFA runs
at module boundary (or lazily on first narrowing demand for a HO function).
