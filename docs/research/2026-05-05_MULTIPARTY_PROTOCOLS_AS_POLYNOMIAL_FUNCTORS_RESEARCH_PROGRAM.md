...
#### §VI.1.9 McDermott–Yoshida 2026: denotational reasoning via grading as the binary-graded precursor (load-bearing late addition)

[McDermott Yoshida 2026], "Denotational reasoning for asynchronous multiparty session types" (arXiv:2604.10646v1, 12 April 2026, Oxford), is the single most relevant published precursor to the architectural target this artifact develops, and warrants its own subsection in the frontier scan despite arriving as a late-noticed reference. The agents conducting the Phase-1 frontier sweep focused on POPL 2026 / ECOOP 2025 / CAV 2023+ and missed the April 2026 arXiv preprint; the reference was surfaced after the synthesis chapter's first draft and is incorporated retrospectively in this revision.

The paper's load-bearing claim, in the authors' own words: "We provide the first denotational semantics for asynchronous multiparty session types with precise asynchronous subtyping... Our development crucially relies on modelling message-passing as a computational effect. We apply grading, a paradigm for tracking computational effects, to asynchronous message-passing, **demonstrating that multiparty session typing can be viewed as an instance of grading.**" Their type system is, in their own framing, "not based on linear logic, unlike previous session type systems"; they use grading instead.

The technical content alignment with the architectural target is striking. Six points are load-bearing:

1. **Computation trees as polynomial-endofunctor coalgebras.** McDermott-Yoshida's central technical device is the *computation tree*, defined coinductively from the grammar `t ::= return(x) | send_{p,m}(t) | recv_p (t_m)_{m∈M}`. This is, structurally, a coalgebra for a polynomial endofunctor: positions = `{return, send, recv}`, with directions varying per position (return has zero, send has one, recv has `|M|` directions). They do not use the polynomial-functor vocabulary — they call their constructions "computation trees" and reason coinductively without invoking Spivak-Niu — but the structure they construct IS a polynomial-functor coalgebra in the sense of §V.5 / §IV.5 of this artifact. This is independent published validation of the coalgebraic-behaviour-as-polynomial-functor-coalgebra commitment.

2. **Grading via graded monad.** The normal forms of typed computation trees form a graded monad `N` where the grade is the session type. Bind `≫=` lifts grades by graded composition `T · T'`. This is "session types as grades on computations" — a different categorical incarnation of "session types as algebraic operations on effects" than the linear-logic correspondence supplies. They credit Katsumata 2014 (parametric effect monads) and Smirnov 2008 (graded monads); their construction is the graded-monad realisation of asynchronous MPST.

3. **Adequacy theorem.** They prove that computation trees provide an *adequate denotational semantics* for SafeMP (their call-by-value asynchronous message-passing calculus), with the operational semantics matching the denotational interpretation up to typed bisimilarity. Communication safety, deadlock-freedom, and liveness are denotational invariants — properties of the model, not extra type-system stipulations.

4. **Reformulated asynchronous subtyping.** Their definition of asynchronous subtyping reformulates Ghilezan-Pantović-Prokić-Scalas-Yoshida 2023's precise asynchronous subtyping using two role-indexed predicates and two role-indexed relations: `Sends_p(T)` ("an implementation of `T` is required to send a message to `p` and is not permitted to wait first"), `Recvs_p(T)` ("an implementation of `T` is permitted to wait for a message from `p`"), and the permitted-send and permitted-receive relations `T ⇝ T'`. This avoids session-tree machinery and enables subtyping for non-closed session types. The role-indexed predicates and relations are the structural data a fibrational role-projection picture (§V.3) would categorify.

5. **Explicit acknowledgement of the open quantale-graded extension.** McDermott-Yoshida flag explicitly: "Our adequacy result (Corollary 20) may follow from a graded variant of Kavvos's result, but there is none in the literature yet." The quantale-graded extension of their construction — replacing their plain-session-type grade with a quantale-valued (e.g., session-type × tropical-cost) grade — is precisely the gap the architectural target's quantale-enrichment fills.

6. **Acknowledgement of binary scope.** Their SafeMP is a call-by-value calculus with non-dependent grades; their type system treats global state as an *example* (their §2 example 2, emulating mutable state via message-passing à la effect handlers) rather than as a primitive. Multi-party participation in their setting is encoded via the participants set being part of the protocol type, not via per-role direction factoring of an upstairs polynomial functor.

What McDermott-Yoshida give the architectural target, in compressed form:

- A working binary-graded denotational existence proof (their adequacy theorem) for asynchronous MPST with precise subtyping.
- Independent validation that "multiparty session typing as instance of grading" is a productive framing — the same framing the architectural target generalises with quantale enrichment and the polynomial-functor lift.
- Computation trees as polynomial-endofunctor coalgebras — the binary-case existence proof for §V.5's coalgebraic-behaviour-as-polynomial-functor-coalgebra hypothesis.
- A reformulated asynchronous subtyping with role-indexed predicates that are the operational shape of fibrational role projection (§V.3).
- An explicit gap statement (the missing graded variant of Kavvos-style adequacy) that the architectural target's quantale extension fills.

What the architectural target adds beyond McDermott-Yoshida, in compressed form:

- Multi-party with per-role direction factoring `B(a) = Π_{r ∈ Roles} B_r(a, v)` (their grade is a single session type; the architectural target has per-role direction structure).
- Quantale enrichment (their grade is plain session type; the architectural target lifts to quantale-graded for cost-aware reasoning).
- Polynomial-functor explicit lift to LCCC with Σ/Π adjoints (their computation trees are coalgebras for a polynomial endofunctor on Set; the architectural target lifts to LCCC for dependent-types-in-continuation).
- Fibrational role projection (their `Sends_p` / `Recvs_p` are role-indexed predicates; the architectural target categorifies them as opcartesian lifts).
- Conjunctive composition / bundle algebra unification (their graded-monad composition is sequential; the architectural target identifies the conjunctive-meet vs sequential-monoidal distinction explicitly).
- Propagator-network operational substrate (theirs is a metalinguistic interpretation; the architectural target is the runtime).

The §V.6 gap inventory is updated to reflect this:

- **Gap (a)** (multi-party session types as polynomial functors over LCCC): McDermott-Yoshida give the binary-graded coalgebraic-polynomial-functor case implicitly; the explicit LCCC lift and the multi-party extension remain open.
- **Gap (d)** (cost-aware multi-party protocols): McDermott-Yoshida supply the binary-graded denotational substrate; the quantale-graded extension is the gap.

The synthesis chapter's overall calibration is *materially strengthened* by this reference: H2 upgrades from "strongly supported" to "settled in the binary-graded case"; H3 upgrades on the coalgebraic-behaviour side from "supported with caveats" to "strongly supported"; H4 acquires a sharper formulation as the quantale-graded extension of an existing binary-graded adequacy result. The architectural target moves from "candidate worth testing" to "candidate that strictly generalises a recent published result; the three-axis extension (multi-party + quantale-enriched + fibrational) is the remaining design-track work."

The paper is also a strong validation of the *direction* of the architectural target. Two leading session-type researchers (Yoshida being one of the original authors of MPST as well as the lead author of the post-2019 reformulation effort) have independently arrived at the conclusion that *grading the computational-effect approach* is the right move, and that the linear-logic correspondence is *not* the only foundation for session types. This is the same conclusion the architectural target reaches, with the further architectural commitments being the lift to multi-party + quantale-enriched + polynomial-functor + fibrational structure.
...
No published 2024–2026 paper formulates multi-party session types as polynomial functors over a locally-cartesian-closed category in dependent type theory *explicitly*. The closest published works approach this from non-MPST directions: [Niu Spivak 2024] supplies the polynomial-functor theory of interaction without applying it to MPST; [Aberlé Spivak 2024–2026] supplies the polynomial-universes-in-DTT framework without addressing concurrent / multi-party computation; [Grodin 2024 / Topos] supplies the polynomial-functor theory of effect handlers without lifting to multi-party. The MPST literature side does not engage with polynomial-functor theory *by name*. The closest *implicit* approach is [McDermott Yoshida 2026] (see §VI.1.9): their computation trees are structurally polynomial-endofunctor coalgebras, and their adequacy theorem is the binary-graded existence proof for the coalgebraic side of the architectural target. They do not lift to the polynomial-functor vocabulary, to multi-party with per-role direction factoring, or to LCCC for dependent-types-in-continuation; the explicit lift and the multi-party-with-quantale-enrichment generalisation remain open.
...
No published multi-party-session-type framework has cost-aware realizability as a *primary* feature. [McDermott Yoshida 2026] (§VI.1.9) supplies the binary-graded denotational substrate — their graded monad `N` of computation-tree normal forms, with session types as grades, has an adequacy theorem covering communication safety, deadlock-freedom, and liveness as denotational invariants — but their grade is plain session type, not quantale-valued. They explicitly flag the absence of a graded variant of Kavvos-style adequacy ("Our adequacy result may follow from a graded variant of Kavvos's result, but there is none in the literature yet"); the quantale-graded extension is exactly the gap. The fairness frontier ([Bravetti Padovani Zavattaro 2025; Padovani Zavattaro 2025]) is sound-and-complete at last but is purely qualitative (does the protocol terminate fairly?). The probabilistic-sessions frontier ([Compositional Interface Refinement Through Subtyping in Probabilistic Session Types ICTAC 2025; Lanese Lago Choudhury 2024 quantum sessions]) addresses probability but not general quantale-graded cost. The tropical-lambda-calculus frontier ([Barbarossa Pistone 2024–2025]) is tropical-quantitative but single-thread. The resource-bounded TT frontier ([Mannucci Thuro 2025–2026]) is graded-quantale but single-program. No published work combines these into a tropical-quantale-graded multi-party-protocol theory; McDermott-Yoshida's binary-graded result is the closest existing substrate the quantale extension would build on.
...
[McDermott Yoshida 2026] McDermott, Dylan; Yoshida, Nobuko. "Denotational reasoning for asynchronous multiparty session types." arXiv:2604.10646v1, 12 April 2026. University of Oxford. **Load-bearing late-arrival reference for this artifact (see §VI.1.9).** The first denotational semantics for asynchronous MPST with precise asynchronous subtyping. Models message-passing as a computational effect; applies grading (graded monads, Katsumata-Smirnov-Mellies tradition) to multi-party session typing, demonstrating that "multiparty session typing can be viewed as an instance of grading." Introduces *computation trees* (`t ::= return(x) | send_{p,m}(t) | recv_p (t_m)_{m∈M}`) — coinductively defined coalgebras for what is structurally a polynomial endofunctor (positions = {return, send, recv}, directions vary per position). Proves adequacy of the denotational interpretation with respect to typed bisimilarity for SafeMP (a call-by-value asynchronous message-passing calculus). Reformulates Ghilezan-Pantović-Prokić-Scalas-Yoshida 2023's precise asynchronous subtyping using role-indexed predicates `Sends_p(T)`, `Recvs_p(T)` and relations `T ⇝ T'`, avoiding session-tree machinery and enabling subtyping for non-closed session types. Type system is *not based on linear logic* (explicit framing), unlike most session-type systems; instead grades sessions over a fine-grain call-by-value λ-calculus. Communication safety, deadlock-freedom, and liveness are denotational invariants. Explicitly flags the absence of a graded variant of Kavvos-style adequacy for computational effects — exactly the gap the architectural target's quantale enrichment fills. The binary-graded existence proof for the coalgebraic-behaviour-as-polynomial-functor-coalgebra commitment underlying the architectural target's H3 hypothesis; the closest published precursor to the multi-party-quantale-graded extension this artifact proposes.
...
A late-arrival reference that directly supports the structural shape of the fibrational picture: McDermott-Yoshida's 2026 *Denotational reasoning for asynchronous multiparty session types* (arXiv:2604.10646, April 2026) reformulates Ghilezan-Pantović-Prokić-Scalas-Yoshida 2023's precise asynchronous subtyping by replacing the session-tree machinery with two role-indexed predicates and two role-indexed relations: `Sends_p(T)` ("an implementation of `T` is required to send a message to `p` and is not permitted to wait first"), `Recvs_p(T)` ("an implementation of `T` is permitted to wait for a message from `p`"), `T ⇝ T'` for permitted sends, and the dual for permitted receives. These predicates and relations are role-indexed structural data extracted from the session type directly, without an intermediate session-tree unfolding step. This is operationally what the fibrational role-projection picture would compute: each role's fibre carries the per-role view, and the predicates `Sends_p`, `Recvs_p` are the inhabitants of that fibre's role-specific behavioural constraint. The McDermott-Yoshida construction does not lift to an explicit fibration, but it identifies the right structural primitives; categorifying them as opcartesian lifts in a fibration over the participant lattice is the move the architectural target makes.
...
The strongest published evidence that this binary-graded coalgebraic picture is operationally sound is McDermott-Yoshida's 2026 paper (arXiv:2604.10646). Their *computation trees* are defined coinductively from the grammar `t ::= return(x) | send_{p,m}(t) | recv_p (t_m)_{m∈M}` — coalgebras for what is structurally a polynomial endofunctor whose positions are `{return, send, recv}` and whose directions vary per position (return has zero, send has one, recv has `|M|` directions). McDermott-Yoshida do not use the polynomial-functor vocabulary, but their computation trees ARE polynomial-functor coalgebras in the architectural target's sense. They prove an *adequacy theorem*: every typed computation tree has a normal form unique up to typed bisimilarity; the normal forms form a graded monad `N` whose grade is the session type; the operational semantics of their call-by-value asynchronous message-passing calculus SafeMP is adequate with respect to this denotational interpretation; communication safety, deadlock-freedom, and liveness are denotational invariants. This is the binary-graded version of the coalgebraic-behaviour picture the architectural target generalises. McDermott-Yoshida supply the existence proof; the architectural target's contribution beyond their work is (i) the explicit polynomial-functor lift to LCCC, (ii) the multi-party-with-quantale-enrichment generalisation, and (iii) the fibrational role-projection picture. Their construction grounds the synthesis chapter's claim that the polynomial-functor coalgebraic picture has a working denotational realisation.
...
The technology stack to make this rigorous is in place, per slice §IV.6.4: Bacci-Mardare-Panangaden-Plotkin 2023 (Lawvere-quantale propositional logic) supplies the quantitative reasoning machinery; Beohar-Gurke-König-Messing-Forster-Schröder-Wild STACS 2024 (quantale-valued logics for coalgebras) supplies the coalgebraic-quantitative bisimulation theorems via Galois-connection-induced fixpoint preservation; Goncharov-Hofmann-Nora-Schröder-Wild FoSSaCS 2023 (Kantorovich functors) supplies the canonical lifting of metric structure through behaviour functors; Kurz CALCO 2025 (uniform type constructors on quantale-enriched categories) supplies the type-theoretic side; Dahlqvist-Neves 2022 supplies the autonomous categories enriched over generalised metric spaces. McDermott-Yoshida 2026 (arXiv:2604.10646) supplies the binary-graded denotational substrate — their graded monad `N` of computation-tree normal forms is the un-quantale-enriched special case the quantale extension generalises. Their adequacy theorem covers communication safety, deadlock-freedom, and liveness as denotational invariants; the quantale-graded extension adds cost-awareness as another projection, with the multi-dimensional Pareto reasoning coming from the product quantale. McDermott-Yoshida explicitly flag the absence of a *graded* variant of Kavvos-style adequacy for computational effects in the literature ("Our adequacy result may follow from a graded variant of Kavvos's result, but there is none in the literature yet"); the quantale-graded extension fills exactly this gap. The synthesis is the recognition that combining these with the polynomial-functor + propagator-substrate framework gives cost-aware multi-party protocol realizability as a single fixpoint computation; nobody currently has this combination in the published literature, and McDermott-Yoshida's binary-graded result is the closest existing approximation.
...
**Gap (a) — Multi-party session types as polynomial functors over LCCC.** No published 2024–2026 work formulates multi-party session types as polynomial functors in dependent type theory *explicitly*. The closest works approach from non-MPST directions: Niu-Spivak's interaction-theoretic polynomial functors, Aberlé-Spivak's polynomial universes in HoTT, Grodin's polynomial-functor effect handlers. McDermott-Yoshida 2026 (arXiv:2604.10646) gives the binary-graded coalgebraic picture *implicitly* (their computation trees are coalgebras for what is structurally a polynomial endofunctor) but does not lift to the polynomial-functor vocabulary or to multi-party with quantale enrichment. The architectural target makes the polynomial-functor identification explicit and lifts to LCCC.
...
**Gap (d) — Cost-aware multi-party protocols.** No published multi-party-session-type framework has cost-aware realizability as a primary feature. McDermott-Yoshida 2026 (arXiv:2604.10646) provides the binary-graded denotational substrate (computation trees as a graded monad, with session types as grades), with safety/deadlock-freedom/liveness as denotational invariants — but their grade is plain session type, not quantale-valued. The tropical-quantale-graded extension is precisely what their construction lacks; they explicitly note that a graded variant of Kavvos-style adequacy "is none in the literature yet." The architectural target supplies the quantale-graded extension via the existing Prologos cost layer (PPN 4C addendum), turning their binary-graded denotational result into a cost-aware multi-party-graded one.
...
**(H2) Polynomial-functor composition product is the categorical realisation of conjunctive refinement at the protocol level.** *Settled in the binary-graded case (McDermott-Yoshida 2026); the multi-party-graded case with quantale enrichment is the remaining verification.* The Spivak-Niu monograph [Niu-Spivak 2024] establishes the composition-product structure formally; the explicit positional formula `(a, f : B_P(a) → A_Q)` with directions `Σ_{b ∈ B_P(a)} B_Q(f(b))` supplies the operational reading; the bicategorical structure (slice §IV.5.5) ensures associativity and unit. The MCC coherence-as-n-ary-duality framing [Carbone-Lindley-Montesi-Schürmann-Wadler 2016] is the syntactic linear-logic shadow of this categorical structure (slice §III.6). McDermott-Yoshida 2026 (arXiv:2604.10646) gives a graded-monad denotational realisation for the binary asynchronous case, with adequacy theorem, where composition is graded-monad bind (`≫=`) over computation-tree normal forms — the graded composition `T · T'` they define on session types is operationally the binary-graded version of the polynomial-functor composition product `◇`. What is not yet demonstrated, and is design-track work, is (i) lifting their graded-monad result to the multi-party case with per-role direction factoring and (ii) extending the grade from plain session type to quantale-valued grade. The implementation of polynomial-functor protocol composition on the propagator network is also design-track work; whether it is performant under realistic load is a question their work does not address.

**(H3) Polynomial functors over LCCC are the right denotational setting for multi-party protocols, with role projection as opcartesian lift in a fibration over the participant lattice.** *Strongly supported on the coalgebraic-behaviour side (McDermott-Yoshida 2026); fibrational role projection remains open.* The polynomial-universes work [Aberlé-Spivak 2025; Awodey-Hofmann-Streicher-Spivak] establishes the categorical machinery; session coalgebras [Keizer-Basold-Pérez 2020] establish the binary-case grounding. McDermott-Yoshida 2026's computation trees are *coalgebras for a polynomial endofunctor* in the architectural target's sense (positions = {return, send, recv}, with directions varying per position); their adequacy theorem establishes that this coalgebraic representation is sound and complete for asynchronous binary message-passing with precise asynchronous subtyping. This is the binary-case existence proof for the coalgebraic-behaviour-as-polynomial-functor-coalgebra commitment. The choreographic-programming line (slice §III.8) provides operational evidence that role projection is implementable. The remaining caveat is the fibrational picture: no published work formulates *multi-party* session types with the per-role direction factoring as opcartesian lifts in a fibration over the participant lattice. The architectural target lifts the binary McDermott-Yoshida coalgebraic structure to the multi-party fibrational structure; the soundness argument that conjunctive composition preserves opcartesian-lift coherence is structurally clean but not formalised in the published literature. The design tracks must produce that formalisation, building on McDermott-Yoshida's binary adequacy as the starting point.

**(H4) Realizability as a quantale-valued fixpoint computation in the propagator network is a strict generalisation of the existing soundness machinery, with cost-awareness coming for free.** *Plausibly supported but not demonstrated; binary-graded special case settled (McDermott-Yoshida 2026).* The technology stack (Bacci-Mardare-Panangaden-Plotkin 2023, Beohar et al. STACS 2024, Goncharov et al. FoSSaCS 2023, Kurz CALCO 2025, Dahlqvist-Neves 2022) is in place; the Prologos infrastructure (effect-ordering quantale, tropical-quantale cost layer) is in place. McDermott-Yoshida 2026's adequacy theorem establishes that the binary-graded (non-quantale) case yields a working realizer for asynchronous message-passing with safety/deadlock-freedom/liveness as denotational invariants — this is the strongest published evidence that the graded-monad / coalgebraic-behaviour combination *does* compose cleanly in the binary case. They explicitly flag that the *graded* extension of Kavvos-style adequacy is missing from the literature; the quantale-graded extension is the precise contribution the architectural target makes. What is not in place is the demonstration that the quantale-graded multi-party extension of their construction yields a working multi-party protocol realizer — that is design-track work. The strongest counter-argument a hostile reader could still mount is that the combination of components might not compose cleanly in the quantale-graded multi-party case (the König et al. 2024 compositionality result for Kantorovich lifting under functor composition does not, by itself, guarantee that polynomial-functor lifting preserves the right structure); the architectural target's claim is that the structural pieces fit; the design tracks must verify they fit operationally. McDermott-Yoshida's binary-graded result is the existence proof that the simpler case works; the multi-party-quantale-graded case generalises within the same algebraic framework.
...
The five formulations together constitute the research program the architectural target implies. After the McDermott-Yoshida 2026 update: (H1) is settled by the algebraic-soundness argument; (H2) is settled in the binary-graded case (McDermott-Yoshida adequacy) with the multi-party-graded extension as design-track work; (H3) is strongly supported on the coalgebraic-behaviour side (McDermott-Yoshida's computation trees ARE polynomial-endofunctor coalgebras, with adequacy theorem) and supported-with-caveats on the fibrational role-projection side; (H4) and (H5) are the genuine frontier hypotheses, but (H4) acquires a sharper formulation as "the quantale-graded extension of McDermott-Yoshida's binary-graded adequacy result." The design tracks the project will pursue from this foundation must, severally, produce the evidence that (H3)–(H5) need; McDermott-Yoshida's binary-graded result is now the closest published starting point, and the architectural target's contribution is the three-axis extension (multi-party + quantale-enriched + fibrational-role-projection).
