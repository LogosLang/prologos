...
**Phase 4 (2026-04-30)**: retired hardcoded `'equality` join lookup in property-test functions; per-relation `join-fn` parameter mirrors Phase 3c's `meet-fn` discipline. The third Scaffolding-Hides-Truth data point.
- *State of the world*: `test-distributive`, `test-sd-vee/detailed`, `test-sd-wedge/detailed` accept explicit `join-fn`; callers derive both `meet-fn` and `join-fn` from the SAME relation via `merge-registry`. The wider-sample sweep (depth-1 with binders, 58 samples, 195112 triples per check) becomes meaningful — pre-Phase-4 sweeps mixed lattices.
- *Algebraic posture surfaces*: the type lattice is **distributive on the ground sublattice (depth-0, atoms only) but SD-but-not-distributive on the binder-included sublattice**. Both relations show the same posture. Witness: `(Pi(m1, Bool, Bool), Int, Pi(m1, Int, Bool))` — distributivity fails when one operand is atomic and the others are Pi-typed. Empirically confirms Track 2H's F7 conjecture (design body line 107: "distributivity is conjectured but not yet verified for binder substitution").
- *Refinement*: SD-vee and SD-wedge both confirm on the wider sample, with asymmetric non-vacuity (3.5% vs 91.4%) — the lattice's join is "spreadier" than its meet on these samples. SD-vee declared confirmed is informationally weak; SD-wedge confirmed is strong. Surfaces the **non-vacuity-ratio-as-evidence-strength** observation: boolean property declarations alone are insufficient for honest characterization of what the empirical evidence says.
- *Posture-mismatch with registration*: Track 2H's design body had F7 scope qualifier (ground sublattice only), but the registration at `unify.rkt:96` declared `'distributive prop-confirmed` unconditionally. Phase 4's wider sample refutes the unconditional reading; the registration is over-broad. This surfaces the **Scaffolding-Hides-Scope** sub-form: registration declarations can elide design-body scope qualifiers, hiding the conditional truth.

**Quantale ≠ distributive lattice**: Track 2H's quantale framing (tensor distributes over join) remains intact. Quantale axioms are about the multiplicative tensor, not lattice-meet-distributing-over-join. A lattice can be SD-not-distributive AND a quantale simultaneously — Phase 4 finding clarifies this orthogonality. The conflation in `unify.rkt:96` was real and worth surfacing for future research notes.
