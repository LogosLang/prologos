...
|---------|----------|-------------|------|
| β/δ-reduction realized as e-graph rewrites on the propagator substrate (the PRN §2 prediction CONFIRMED) | The β redex and its contractum share ONE e-class cell ({redex, whnf-result} joined under the componentwise-ACI product merge — `preduce-ingest-delta`/the guarded β clause, reduction.rkt); the rewrite is DPO-style local replacement with the effect-safety guard as the application condition (a guarded match SKIPS — the NAC realized as a structural application condition, not an exception); δ likewise ({def-body, unfolded-whnf} digest-keyed). Literal folds dispatch as declarative registry rules through the same choke point. Tests: test-preduce-ingest.rkt (guarded β fires; effect-capturing β never recorded), test-rules-seed.rkt. | PReduce Track 2 (iterations 19-25, autonomy loop) | 2026-06-10 |
...
| Pocket Universe: cell value IS a lattice | A CHAMP/RRB inside a cell is an embedded lattice with its own entries, merge, diff. NTT `:embedded` kind. Sequential domains use RRB (9× build, 3× read vs CHAMP). Reduces parse state from 5000+ cells to 5 cells. | PPN Track 1 D.6-D.7 | 2026-03-26 |
| Parse tree as M-type in single cell | Tree topology stored as polynomial functor initial algebra (M-type) in ONE cell. Same representation as SRE type trees (expr-Pi). SRE decomposes tree cell for macro rewriting. Tree-union merge is monotone. | PPN Track 1 D.7 | 2026-03-26 |
| Topology transformation (linear→tree) as hypergraph rewriting | Input: linear character sequence. Output: tree of structure nodes. The rewriting TRANSFORMS the graph topology, not just cell values. Persistent data structures mean both representations coexist (no consumption). | PPN Track 1 D.5-D.7 | 2026-03-26 |
