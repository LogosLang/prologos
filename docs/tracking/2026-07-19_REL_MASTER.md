...
|---|---|---|---|---|
| 1 | Relational Language Usability | NAF/guard correctness (priority — `not` echo + on-network NAF single-bit collapse + guard crash) + typed solution rows (codata + schema projection) + schema-as-relational-facts + Aspect D fact-representation research (Stage 0/1) + polish. **Deferred to UCS**: `?v:Type` CLP resolution. | 🔄 Aspect-A: **A.1 + A.2-core + A.2b ✅** (`bcd02d6d`; A.2b = adaptive-dispatch DFS-routing for body-local-var rule NAF; root reframed to the body-local-var gap, worldview-preservation deferred to BSP-LE Track 3). **NEXT A.3** (static floundering). B/C/D open. | [design](2026-07-19_REL_T1_RELATIONAL_USABILITY_DESIGN.md) · [seed](2026-07-19_REL_SOLVE_TYPING_NOTE.md) |
