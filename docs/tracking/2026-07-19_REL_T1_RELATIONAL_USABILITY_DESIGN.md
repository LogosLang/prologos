...
| **SUB** | **POL.10 spin-out** — the substitution containment defect (LIVE bug: open de Bruijn index escapes via runtime containers). Own doc + progress table: [`2026-07-24_SUBSTITUTION_CONTAINMENT_DEFECT.md`](2026-07-24_SUBSTITUTION_CONTAINMENT_DEFECT.md). **Owner-resequenced AHEAD of the POL remainder (2026-07-24)**; ruling **(D)** (champ = closed runtime value ⇒ NbE fix) | 🔄 | R ✅ (D) · SUB.1 ✅ `f19d6f56` · SUB.2 ✅ **ADOPTED** `6323587e` (compile-limit: shift ~830× micro, suite −18%; runner putenv) · **SUB.3a ✅ `7ea49168` — THE FIX** (NbE nf; repro 6N/5N; +6.4% suite, net 211→185) · SUB.3b narrow-subst-bvars ⬜ · SUB.close ⬜ |
