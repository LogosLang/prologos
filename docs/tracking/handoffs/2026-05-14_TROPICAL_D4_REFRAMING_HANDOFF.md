...
**Date**: 2026-05-14 (handoff written today; D.4 scaffolding pass complete; Cell/Propagator/Scheduler Orthogonality principle codified; §13.6 Pre-0 spike pending — the falsification test)
**Note on dates**: This handoff is dated today (2026-05-14) per its creation. Existing design documents in the PPN 4C tropical addendum series (D.1/D.2/D.3/D.4 design doc; D.2.SC; D.3.EC; Pre-0 plan; dailies file) carry `2026-04-26` filenames because they're tied to the original design-work series start date; per workflow.md dailies convention ("the 'current' dailies remain the file created at session start"), the dailies file `2026-04-26_dailies.md` continued accumulating session work across multiple calendar days. Internal references to `2026-04-26` in this handoff refer to either (a) the design-work series identifier, or (b) commit-message-internal date stamps from prior sessions — they're historically correct, not the current date. **Today is 2026-05-14**.
