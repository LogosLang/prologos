...
| 3 | A5 `:type` / `:term` facet split | 🔄 | See **§6.15** for full Phase 3 mini-design (2026-04-20). **S1 resolved**: reading (i) — TermFacet IS the SRE 2H quantale; `:type`/`:term` are role-tags over ONE carrier lattice with tag-dispatched merge. SRE lens Q1-Q6 answered in §6.15.1. **P4 resolved**: path (b) — cross-tag merge emits stratum request; merge stays pure `(v × v → v)`. Worldview-tagging of the request is a Phase 3+9 joint mini-design item tracked in §6.15.6. **Sub-phase partition**: 3a+3b atomic (facet infra + tag-dispatched merge), 3c migration of typing-propagators, 3d parity tests + lazy-vs-eager A/B bench, 3e Phase 1f classification to `'structural`, 3V Vision Alignment Gate. **PU audit**: Phase 3 is PU-aligned (attribute-map already compound; only VALUE SHAPE changes). Per-meta-cell consolidation deferred to Phase 4 mini-design. **Drift risks** at §6.15.8. **3a+3b ✅** (commit `98f503a2`): [`classify-inhabit.rkt`](../../racket/prologos/classify-inhabit.rkt) delivers tag-layer struct + pure accumulation merge + `'classify-inhabit` SRE domain (Tier 1 'structural + Tier 2 linkage). Classifier × classifier uses `type-lattice-merge`; inhabitant × inhabitant uses equal? (α-equiv proxy for 3a+3b MVP; 3c refines via ctor-desc). Cross-tag residuation check deferred to 3c as dedicated propagator per P4(b). 19/19 tests GREEN. **3c-i ✅** (commit `f21ed694`): reshape `:type` facet VALUE SHAPE to `classify-inhabit-value` + reader shim + `:term` magic keyword dispatch. Facet registration triple (merge/bot/bot?) updated atomically. Raw-value backward-compat at the boundary via Module-Theory embedding (base → classifier-only). New [`tests/test-facet-tag-dispatch.rkt`](../../racket/prologos/tests/test-facet-tag-dispatch.rkt) 20/20 GREEN. All 8 drift risks from 3c-i mini-design audit cleared. 7911 full-suite tests GREEN at 122.2s. **3c-ii ✅** (commit `08782d1a`): per-rule writer migration — 3 INHABITANT sites migrated to `:term` (trait-resolution dict-expr write, meta-feedback simple case, meta-feedback structural case) via new `term-map-read` / `term-map-write` helpers symmetric with `type-map-read` / `type-map-write`. Bridge migration surfaced by test-hasmethod-01 regression: `make-meta-solution-output-fire-fn` reads `:term` (not `:type`) — meta solutions live in INHABITANT. 7914 full-suite tests GREEN at 119.8s. **3c-iii ✅** (commit `2e768b0d`): cross-tag residuation propagator + stratum handler per Q2. cell-id 10 = classify-inhabit-request (pre-allocated in make-prop-network, next-cell-id 10→11). `type-of-expr` helper classifies literals + type-constructors. `make-classify-inhabit-residuation-fire-fn` threshold-fires; uses `subtype?` predicate (semantically correct for compatibility check; `subtype-lattice-merge` returns union for incomparable types not type-top). Contradiction write is the `'classify-inhabit-contradiction` sentinel; merge absorbs via existing path. Stratum handler registered; Phase 9 consumes request cell for ATMS-tagged fork-on-narrowing per §6.15.6 joint item. Install parallel to meta-solution-output at install-typing-network. New [`tests/test-residuation-propagator.rkt`](../../racket/prologos/tests/test-residuation-propagator.rkt) 15/15 GREEN including **Option C skip dissolution verification** (§6.15.9 #8): Type(0) classifier + Nat inhabitant resolves cleanly. test-propagator / test-observatory / test-trace-serialize updated for cell-id 10→11 cascade. 7929 full-suite tests GREEN at 126.3s. **3d-parity ✅** (commit `ad3f0218`): [`tests/test-elaboration-parity.rkt`](../../racket/prologos/tests/test-elaboration-parity.rkt) wired up with approach (C) — expected-output assertions (no dual-path code, no git-checkout orchestration). Axis 5 tests enabled: `type-meta-split/literal` (baseline 3N→Nat), `/identity-app` (poly identity [(fn [x] x) 3N] → Nat), `/int-identity` ([(fn [x] x) 42] → Int). Other axes stay SKIP-gated via new `parity-test-skip` macro. 7931 full-suite tests GREEN at 124.1s. **3d-bench ✅** (commit `52432eff`): A/B microbench per §6.2 verification plan using bench-micro.rkt statistical harness. Three workloads × 100k fire-fn calls × 15 samples. Results: all-unchanged (adversarial) +950% eager overhead → adopt LAZY (but not representative); all-changed -3.9%, half-changed -3.4% → eager FASTER on representative workloads. **Decision: ADOPT EAGER** (current 3c-iii impl stands) per §6.2 "<5% overhead → adopt eager." Eager is also PRINCIPLED (stateless per propagator-design.md); closure-cache lazy violates that; cell-based lazy adds architectural weight for no representative-workload payoff. Observability note in bench header: if `prop_firings` counter shows high redundant-fire counts in real elaboration, re-run with companion-cell lazy variant. **3e ✅** (commit `9ffd9df0`): classify `'attribute-map` SRE domain 'structural; register/minimal gains `#:classification` kwarg. 5 propagator sites fixed: variadic literal (declarable paths list); literal-fire, constraint-creation, warning-collection + 2 test manual net-add-propagator calls (whole-cell via `(cons tm-cid #f)` — explicit intent, passes Phase 1f via filter convention already at propagator.rkt:1117). Design question resolution (§6.15.5): warning-collection accepted as whole-cell with doc rationale (S2 aggregation emits warning positions dynamically). Scope smaller than feared (5 fixes, not 10+). Follow-up: remove tm-cid inputs from fire-once propagators that don't actually read it. 7931 full-suite tests GREEN at 123.2s. **Addendum** (commit `716bc923`): `that-read` gains arity-2 whole-record view (case-lambda dispatch). Returns user-facing hash with `:type` facet's classify-inhabit-value decomposed into `:type` (classifier) + `:term` (inhabitant) entries. 10 new tests. Forward-scheduled for Phase 11b diagnostic (`derivation-chain-for`), LSP hover/inspection, and user-facing `that` grammar form. **3V ✅** (Phase 3 COMPLETE): all 4 VAG questions pass. **(a) On-network**: classify-inhabit-value IS cell value; merge via facet-merge dispatch; residuation is real propagator; stratum handler registered; Phase 1f enforcement live. NO off-network state introduced. Scaffolding labeled: `upgrade-to-classify-inhabit` Module-Theory boundary wrapper; `(cons tm-cid #f)` whole-cell declarations with documented follow-up. **(b) Complete**: all §6.15.3 sub-phases (3a+3b, 3c-i/ii/iii, 3d, 3e, 3V) landed; 3c-iv α-walk honestly deferred per §6.15.8 Q5 observational trigger. **(c) Vision-advancing**: Module Theory Realization B realized — ONE quantale carrier with tag-dispatch; Option C skip DISSOLVED; residuation via `subtype?` (simpler than `subtype-lattice-merge`, semantically correct for MEET-as-compatibility); parity regression-gate via approach C (expected-output assertions, no dual-path debt); lazy-vs-eager data-locked. **(d) Drift-risks-cleared**: all 8 §6.15.9 risks + all 7 §6.15.10 risks cleared. TermInhabitsType artifacts grep-verified ZERO. Phase 3 aggregate: ~12 commits, ~370 LoC production (classify-inhabit.rkt + typing-propagators.rkt + propagator.rkt changes), 57 new tests, full suite stable at 124s (~7941 tests, +~30 from Phase 3 start). |
| 4 | A2 CHAMP retirement | ⬜ | **Resequenced after Phase 9** (framing B decision 2026-04-20): Phase 9 ships cell-based TMS substrate FIRST; Phase 4 then retires both the meta-info CHAMP AND the per-meta TMS cells onto Phase 9's substrate. β2 absorbed into Phase 4's scope (attribute-map `:type` facet becomes sole meta store; no separate A2-II follow-up). Rationale: (i) β2 is the architecturally complete position the track's vision demands; (ii) framing A's α + A2-II creates a duplication window (per-meta cells + attribute-map coexisting for months) that is "discipline-maintained consistency" — the exact smell workflow.md flags; (iii) Phase 9 is a substrate phase whose output is leveraged by multiple downstream phases (Phase 4 β2, 9b, 10) — front-loading compounds value; (iv) Phase 4's own mini-design is LIGHTER once Phase 9 is in place (no α/β/γ architecture choice to re-litigate — β2 is the landing shape). Pre-Phase-9: meta-info CHAMP (off-network) + per-meta TMS cells (on-network) + attribute-map :type facet (on-network). Post-Phase-9 Phase 4: attribute-map :type facet ONLY (single authoritative store, worldview-tagged via Phase 9 substrate). Mini-design at Phase 4 open: audit final migration scope (readers across ~17 files + ~160 sites + 513+ zonk.rkt sites), sub-phase partition, drift risks. **Tag-layer scheme from Phase 3** composes with β2 cleanly (meta's type lives in :type facet CLASSIFIER layer; solution in INHABITANT layer). **Cache-field + callback retirements absorbed into Phase 4 scope (added 2026-04-24 from PPN 4C addendum S2.c-iii mini-audit, see [`2026-04-21_PPN_4C_PHASE_9_DESIGN.md`](2026-04-21_PPN_4C_PHASE_9_DESIGN.md) §7.5.13.6.1)**: (i) `expr-meta-cell-id` field retirement — per S2.c-i Task 1 microbench, the cache field path is ~302ns SLOWER than parameter-read; phantom optimization under universe migration (with-handlers continuation-marker overhead exceeds 80ns id-map savings). Cache field becomes inert post-S2.c-iii (option 4 dispatch never reads it); Phase 4's struct cleanup deletes the field. (ii) `sess-meta.cell-id` field retirement — Track 10B Phase B1b cache field; same phantom-optimization pattern as `expr-meta-cell-id`. Becomes inert post-S2.d session universe migration; Phase 4 struct cleanup deletes. (iii) `current-lattice-meta-solution-fn` callback parameter retirement — off-network scaffolding (Racket parameter holding `meta-solution` function, installed by driver.rkt:2633). type-lattice.rkt's `is-meta-unsolved?`-style checks (lines 86, 176, 399, 403, 421) currently invoke this callback; restructure once CHAMP retires + cells accessible from leaf module via PM Track 12. The callback exists to break import cycle (type-lattice.rkt is leaf, can't import metavar-store.rkt). Mantra violations: ❌ off-network, ❌ not structurally emergent, ❌ not info flow through cells. Retirement cooperative with PM Track 12 (DEFERRED.md tracked); Phase 4 provides the natural reframing point. **Additional retirements absorbed from PPN 4C Addendum Phase 2A (added 2026-05-20 per `2026-04-21_PPN_4C_PHASE_9_DESIGN.md` §8.7.a + §8.7.b scaffolding-retirement tables)**: (iv) `elab-network-meta-info` CHAMP struct field → cell with tagged-cell-value (worldview-filtering at read time replaces explicit `worldview-visible?` checks at consumers). **2A.a (commit `0c25400f`) deferred meta-info retraction work to here**: 2A.a's `process-retraction` handler is pure on prop-net (scoped cells only); meta-info CHAMP retraction is now structural via worldview-filtering. Phase 4's CHAMP→cell promotion replaces the structural worldview-filtering pattern with cell-native tagged-cell-value semantics. (v) `elab-network-id-map` CHAMP struct field — same migration; 2A.a deferred id-map retraction work to here for identical reasons. (vi) **6 worldview-aware reader migration sites from 2A.a** become trivial cell reads post-CHAMP-promotion: `meta-info-solved?` (metavar-store.rkt:2359), `meta-lookup` (:2372), `all-unsolved-metas` (:2957), trait dispatcher (`:617` + `:666`), hasmethod readiness + bridge dep-pairs (`:763` + `:799`), `add-unify-constraint` lhs/rhs (`:971`). Each currently uses `elab-meta-info-read-worldview` / `elab-id-map-read-worldview` (worldview-aware lookup via `champ-lookup-worldview` filter). Post-Phase-4: cell-based meta-info has native worldview filtering via tagged-cell-value — the explicit `worldview-visible?` check at use sites dissolves; readers become trivial `(net-cell-read net meta-info-cid)`. (vii) `with-speculative-rollback`'s elab-net snapshot + `set-box! elab-net-box saved-enet` block at `elab-speculation-bridge.rkt:317-318` — depends on meta-info CHAMP being a cell (then snapshot-restore semantics become cell-restore via worldview-cache bit clear). Coordinates Phase 4 (CHAMP→cell) + PM Track 12 (parameter→cell) for full retirement. (viii) **From PPN 4C Addendum Phase 2A.b** (in flight 2026-05-20): `process-resolution` handler body's `set-box! net-box` + `current-prop-net-box` parameter read + `current-resolution-executor-pure` parameter read — depends on enet/pnet boundary dissolving post-CHAMP-promotion. The handler's box-bridge pattern is the same scaffolding as `with-speculative-rollback`; same Phase 4 + PM 12 retirement path. **Captured here so Phase 4's mini-design absorbs these scope items as it audits the final migration surface.** Cross-reference: **PM Master Track 13** (added 2026-05-20 — [`2026-05-20_PM_TRACK13_IMPLEMENTATION_NOTE.md`](2026-05-20_PM_TRACK13_IMPLEMENTATION_NOTE.md)) addresses the SEPARATE concern of stratum-handler REGISTRY/MECHANISM as scheduler-state cells — orthogonal to Phase 4's body-side-effect retirements. PM 13's research can inform Phase 4's mini-design about which body-side-effects retire here vs which migrate via PM 12/PM 13. |
...
| 10 | Phase 8 union types via ATMS | ✅ | **CLOSED 2026-05-24** — absorbed into addendum Phase 3 (3A + 3B + 3C + 3V); landed at 3V close (this commit). See [addendum §9.6](2026-04-21_PPN_4C_PHASE_9_DESIGN.md) for cumulative cross-PHASE VAG. Realization B (in-place worldview tagging on shared carrier, NOT fork-and-rejoin); 3 sub-phases + cumulative VAG; Hyperlattice Conjecture's CHAMP-sharing claim bounded mechanism-coupled per 3B-VAG (not refuted). KR-1/2/3 named drift escalated to Phase 11b. Original scope text preserved: Fork-on-union, TMS-tagged branches, S(-1) retract. |
| 11 | A7 Elaborator strata → BSP scheduler | ✅ | **CLOSED** — absorbed into addendum Phase 2 (orchestration unification); landed via cross-arc 2V at §8.9. See [addendum §3 row Phase 2V](2026-04-21_PPN_4C_PHASE_9_DESIGN.md). Both orchestrators retired per R3 external critique. Original scope text preserved: S(-1)/S1/S2 as BSP handlers. **Orchestrator retirement (R3 external critique 2026-04-18)**: BOTH orchestrators retire in Phase 11 — `run-stratified-resolution!` (already dead code, zero production callers) deleted as hygienic prelude; `run-stratified-resolution-pure` retired as main work. |
| 11b | Diagnostic infrastructure — residuation-backward error reporting (NEW in D.2) | ⬜ | First-class compiler + error features per user direction 2026-04-18. `derivation-chain-for(position, tag)` helper (API shape phase-time mini-design). Human-readable error messages with source-loc + propagator rationale; machine-readable structured traces for IDE/LSP. Backward residuation over the propagator-firing dependency graph (Module-Theory-principled, not ad-hoc tracker). See §6.1.1. **M4 external-critique lean (2026-04-18)**: read-time derivation (option b) — no error-propagator fires; `derivation-chain-for` is a read-time function over the dependency graph. **Research input**: trace monoidal category theory (Joyal-Street-Verity 1996; Hasegawa 1997; Abramsky-Haghverdi-Scott 2002) — network forms traced SMC, provenance IS trace morphism, backward residuation IS adjoint structure of trace. Consume before mini-design finalization. **Scope addition (2026-04-19) — Gap B from Phase 1.5 mini-audit**: `expr-*` core AST nodes currently carry NO srcloc field (~50 structs in syntax.rkt). Phase 11b's backward-residuation anchors need srcloc at post-elaboration nodes for precise source mapping. Decide at Phase 11b mini-design: (a) add srcloc field to all `expr-*` structs (pipeline.md cascade — 50 structs + struct-copy audit), or (b) side-channel mapping `(expr-node-identity → srcloc)` maintained during elaboration, or (c) Phase 1.5's propagator-struct srcloc proves sufficient via cell provenance tags without needing per-expr-node srcloc. Resolution informed by actual Phase 11b use cases. **Scope addition (2026-04-20) — identity-or-error upgrade from Phase 1e (Q4 resolution)**: Phase 1e replace-cell audit uses path (A) sentinel + `#:contradicts?` predicate for identity-or-error merges in the short term. Phase 11b upgrades identity-or-error sites to path (C) provenance-rich contradiction descriptors — returning a structured contradict-record carrying the conflicting values + their srclocs + producer propagator IDs. When `derivation-chain-for` fires on an identity-or-error contradiction, the chain starts with the two conflicting writes' provenance. Upgrade is non-breaking (the contradict-record's `contradicts?` predicate still returns true; the sentinel just carries more data). Captured here per Phase 1e Q4 resolution 2026-04-20. **Scope addition (2026-05-24) — KR-1/2/3 absorption from PPN 4C addendum 3C.c-VAG + 3C.d.4**: Phase 11b inherits 3 named drift items from addendum 3C.c-VAG (per [addendum §9.5.4.14](2026-04-21_PPN_4C_PHASE_9_DESIGN.md) + [§9.5.5](2026-04-21_PPN_4C_PHASE_9_DESIGN.md) + [§9.5.5.13](2026-04-21_PPN_4C_PHASE_9_DESIGN.md)). **(KR-1) Diagnosis-line UX regression**: pre-3C.c rendering of `(def x <Nat | Bool> "hello")` produced `[diagnosis] retract: x : <Nat | Bool>` via `format-context-diagnosis` (typing-errors.rkt:173-280). Post-3C.c shape `(listof derivation-chain)` renders per-step `because: <name>` only — diagnosis lines disappear. **(a)/(b)/(c) decision history (3C.d.0 Q-D.1 lock)**: (a) RESTORE-at-3C.d via ATMS state queries — REJECTED (inverts 3C.c intent; render-time staleness; KR-3 grows); (b) ESCALATE-with-timeline — REJECTED (unbounded "pragmatic" deferral anti-pattern); (c) ESCALATE-with-substrate — LOCKED. Phase 11b mini-design RE-WEIGHS (a)/(b)/(c) with trace-monoidal-category-theory framing as research input. Restoration substrate (~50-100 LoC mechanical glue) still live for non-union path (typing-errors.rkt:173-280); union path restoration is the work. Skip-gated canary at tests/test-elaboration-parity.rkt (`'union-diagnosis-restoration "Phase 11b"`) preserves regression-detection; DEFERRED.md PPN 4C section tracks formally. **(KR-2) Derivation-step field sparsity for sexp-fed steps**: 2/5 fields (`propagator-id` + `srcloc`) are `#f` for sexp-translator-produced steps. Enrichment naturally lands when Phase 11b's general `derivation-chain-for(position, tag)` primitive replaces the sexp translator (coordinate with Track 4D attribute-grammar substrate unification per PPN Master). **(KR-3) Two-shape error infrastructure**: `union-exhaustion-error.derivation-chain` (new shape per 3C.c) + `type-mismatch-error.provenance` (old shape, still live for non-union path) coexist. Phase 11b's general primitive unifies both shapes atomically with retirement. **Chain-rendering-modes consideration (3C.d.2 (γ+))**: when Phase 11b adds new chain rendering modes (LSP-serialized, trace-monoidal-category-theory framing for general derivation-chain-for, non-union chain unification), weigh whether to (i) add axis-named tests (T-D.* convention) at THAT time, (ii) continue cross-reference-comment pattern at T-C.c-1.4/1.5/1.6, (iii) refactor T-C.c-1.* to axis-aligned naming. Decision conditional on Phase 11b's actual rendering-mode scope. **3C.d.3a empirical probe artifact** (`racket/prologos/data/probes/2026-05-24-3C-d-3-w1-empirical-{probe.rkt,output.txt}`): re-runnable baseline for empirical W1 srcloc state at 3C.d.3 close. Phase 11b mini-design uses it for (i) verifying current dispatch-chain srcloc state; (ii) comparing pre-Phase-11b vs post-Phase-11b srcloc threading; (iii) auditing production sexp-path srcloc readiness. Probe demonstrates W1 dispatch-chain coherence mechanically (Scenario B) AND empirically confirms sexp translator hardcodes srcloc=#f BY DESIGN (Scenarios C1/C2 — D-3C.c-1). |
...
Phase 3 (A5 :type/:term split)
  ↓
Phase 9 (BSP-LE 1.5 TMS) — sub-track; RESEQUENCED AHEAD OF PHASE 4 (framing B, 2026-04-20);
  substrate for Phase 4 β2, Phase 9b, Phase 10. Depends on Phase 1 infra + Phase 3 :type facet.
  ↓
Phase 4 (A2 CHAMP retirement) — β2 scope absorbed (attribute-map becomes sole meta store);
  depends on :term facet (Phase 3) + cell-based TMS substrate (Phase 9)
  ↓
...
Phase 7 (A1 parametric resolution) — uses Phase 2b Hasse-registry
  ↓
Phase 8 (A4 Option A freeze) — depends on CHAMP retirement (Phase 4); low-priority scaffold
  retired in Phase 12. Could parallel with later phases.
  ↓
