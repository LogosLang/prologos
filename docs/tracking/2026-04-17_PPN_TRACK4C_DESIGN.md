...
| 2b | Hasse-registry primitive | ⬜ | Thin wrapper on existing infrastructure — `infra-cell.rkt`'s `net-new-registry-cell` provides the cell; SRE+PUnify provides the Hasse structural walk via L's `'subtype` relation + ctor-desc. ~200-250 lines total (primitive + SRE reg + tests). Default subsume-fn = PUnify invocation; specialized lattices (e.g., Q_n hypercube bitmask per prior art) override. Generalizes existing Hasse practice (tagged-cell-value across 10+ files; ATMS subcube bitmask). No materialized edges — Hasse order implicit in L's `'subtype` relation. Foundational for Phase 7 (L_impl) + Phase 9b (L_inhabitant) + future tracks (PPN 5, FL-Narrowing, PM trait coherence). See §6.12 (refined 2026-04-19 per Phase 2b design dialogue). |
