...
| 1 | A8 `:component-paths` enforcement via Tier 2 merge-function inheritance | 🔄 | **Phase 1a ✅ (2026-04-19)** — [`tools/lint-cells.rkt`](../../racket/prologos/tools/lint-cells.rkt) created + baseline saved ([`tools/cell-lint-baseline.txt`](../../racket/prologos/tools/cell-lint-baseline.txt)); 101 production sites / 27 unique unregistered merge fns / 1 inline lambda / 6 ambiguous-name / 11 multi-line. **Phase 1b ✅ (2026-04-19)** — [`merge-fn-registry.rkt`](../../racket/prologos/merge-fn-registry.rkt) Tier 2 API: `register-merge-fn!/lattice` + `lookup-merge-fn-domain` per §6.8 option (a) (independent reverse-lookup registry, function-object `eq?` keying, idempotent-same-domain/error-different-domain collision semantics, scaffolding-labeled for PM Track 12). [`tests/test-merge-fn-registry.rkt`](../../racket/prologos/tests/test-merge-fn-registry.rkt) 8/8 GREEN. **Phase 1c ✅ (2026-04-19)** — `#:domain` keyword extended to `net-new-cell`, `net-new-cell-desc`, `net-new-cell-widen`; batch inherits automatically via `lookup-merge-fn-domain` on each spec's merge-fn. Storage: `cell-domains` CHAMP added to `prop-net-cold` (option (β) — parallel CHAMP, minimal blast radius — 1 struct definition + 1 positional constructor + 1 accessor; 15 `struct-copy prop-net-cold` sites unchanged via struct-copy field preservation). New `lookup-cell-domain` API for Phase 1f. [`tests/test-cell-domain-inheritance.rkt`](../../racket/prologos/tests/test-cell-domain-inheritance.rkt) 10/10 GREEN. Suite: 6017 tests GREEN, 110.6s (no regression). **Tier 1/2/3 architecture** (§6.8). Tier 1 = SRE-registered lattice type with classification (A9 covers the 6 facets). Tier 2 (NEW): `register-merge-fn!/lattice` registers a merge function `#:for-domain DomainName` — links Tier 2 implementation to Tier 1 type. Tier 3: `net-new-cell` inherits domain from merge function; `#:domain DomainName` keyword for explicit override (rare, must be registered). Audit scope: **~37 production merge functions** (not 666 cell sites). Top 10 cover 70% of production calls. `tools/lint-cells.rkt` baselines unregistered merge functions and `#:domain` overrides. Mini-design during Phase 1 for Tier 2 API shape. |
| **1.5** | **srcloc infrastructure (NEW 2026-04-19)** | ⬜ | **Foundational srcloc infrastructure for compiler + tooling.** Unified "current source location" mechanism, API for emit sites (warnings, errors, diagnostics), threaded through elaboration/typing entry points. Gets its own mini-design cycle (scope boundaries strict — see §6.9.1). Forward-enables: `:warnings` correct-first-time registration in Phase 2, Phase 5 warnings authority (srcloc + merge-set-union switch), Phase 11b diagnostic infrastructure (M4), LSP tooling (PM Track 11), error recovery (PPN Track 6), type-directed disambiguation (PPN Track 5). **Ordering (2026-04-19)**: pulled before Phase 2 per Completeness Principle — prerequisite for Phase 2's `:warnings` D1 resolution (position-in-value). Design decision: srcloc storage (Racket parameter vs on-network cell vs `:context` facet field) — resolved at Phase 1.5 mini-design audit time. |
| 2 | A9 facet SRE domain registrations | ⬜ | `context`, `usage`, `constraint`, `warning`, `term` domains; property inference. **D2 framework (2026-04-19)**: each facet ships with an **aspirational / declared / inference-result / delta** analysis table as commit artifact (see §6.9.1 per-facet projected shape). Declare conservatively (γ) — minimal believed properties; inference confirms/refutes; delta drives resolution (fix, accept + document, or defer to named phase). Depends on Phase 1.5 (srcloc) — `:warnings` registered with set-union + position-in-value, correct-first-time. **Lattice-bug contingency (R5 external critique 2026-04-18)**: K=2 bugs absorbed into Phase 2; K+1 (≥3 bugs) opens a Phase 2c repair sub-phase; **zero bugs found mandates a property-coverage review** — prior-track precedent (Track 3 §12 + SRE 2G) predicts at least one; zero means property inference isn't catching what it should. **D1 resolution: `:warnings` as set lattice** — Quantales are not necessarily commutative, but for `:warnings` we adopt set-union with srcloc carried IN value (PPN Track 1/2 "position-in-value" pattern per [structural-thinking.md](../../.claude/rules/structural-thinking.md) § Direct Sum Has Two Realizations). `:context` accepts non-commutative monoidal structure (binding-stack order has scope semantics). |
...
Phase 1a ✅ → Phase 1b ✅ → Phase 1c ✅ (Tier 2 + Tier 3 infrastructure)
  ↓
Phase 1.5 (srcloc infrastructure, NEW 2026-04-19) — prerequisite for Phase 2's `:warnings` correct-first-time registration; forward-enables Phase 5, Phase 11b, LSP, etc.
  ↓
...
Phase 2b (Hasse-registry primitive) — foundation for Phase 7 + Phase 9b
  ↓
Phase 1d → 1e → 1f → 1V (complete Phase 1 — merge-fn registrations, enforcement, hard-error flip)
  ↓
