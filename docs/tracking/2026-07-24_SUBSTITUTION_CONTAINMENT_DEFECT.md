...
| **SUB.3b** | `narrow-subst-bvars` missing arms — the WIDER sibling walker (silent `[_ expr]` catch-all drops bvars for map/set/pvec spines; not fixed by any champ change) | ⬜ | failing-test-first; separate slice (narrowing subsystem) |
| **SUB.close** | Doc-truth (§2 table verdicts now stale post-fix) · lesson promotion (un-arm'd-node 3rd data point; touch≠recompile) · Rel T1 tracker/dailies fold | ⬜ | folds into Rel T1 X.close if owner prefers |
