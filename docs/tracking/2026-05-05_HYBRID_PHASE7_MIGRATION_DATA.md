...
| 6 | **`expr-int-mod`** | **DONE 2026-05-05** | trivial: kernel `kernel_int_mod` at tag 7 + `'int-mod 7` in NATIVE-OP-TAGS + `#:native-op 'int-mod` on the compile case. | int-binary cluster complete. W4 GCD's 3 int-mod fires moved from cb to native; W14 prime-count similar shift. |
