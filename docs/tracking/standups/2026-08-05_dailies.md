...
- **Suite**: **9880 / 483 files / 0 failures** (`all_pass: true`).
  Baseline at branch point: 9847 / 482 / 0. (+33 net; the 51(c) inversions
  replaced five limit-pinning tests with fewer, sharper ones.)
...
- **NEXT**: decide whether to extend 51(c) to the `[else]` preparse arm. A bare
  top-level `rel` with parenless clauses still degrades and still refuses
  (measured) because it does not go through the `defr` arm — so `rel` and `defr`
  now DISAGREE on the same POL.8 grammar. Closing it is the same helper at a much
  wider call site. Then 54, `expr-narrow`, 51(b)'s enriched query message, and
  the subterm-granularity gate.
- **Open/blockers**: none.
