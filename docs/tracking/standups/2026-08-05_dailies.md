...
- **Where we are**: both DEFERRED items are FIXED, but **the value delivered is
  ...
  pre-existing defect.
- **NEXT**: read the targeted corpus A/B when it lands (24 relational corpus files
  + 8 controls, `b429d038` vs HEAD; first run over 93 pairs of the full corpus had
  **zero semantic diffs** — its 4 DIFFERS were gensym drift and the lib PATH inside
  an error message). Then decide on DEFERRED 53 (fabricated fact rows — the most
  severe thing found) and 54. Older note on reading corpus output (161 files, pre-fix `b429d038` vs
