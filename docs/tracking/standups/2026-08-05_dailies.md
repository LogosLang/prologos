...
- **HEAD**: re-derive with `git rev-parse HEAD` — expect `fb75e8e3` or a later
  docs commit. Worktree `.claude/worktrees/wizardly-mendel-2fd502`, branch
  `wizardly-mendel-2fd502`, based on `b429d038`. **21 commits, NOT pushed.**
- **Suite**: **9890 / 483 files / 0 failures** (`all_pass: true`) at the last
  behavioural commit (`134ddb79`; `fb75e8e3` is comment/docs-only on top).
  Baseline at branch point: 9847 / 482 / 0. Read `all_pass` from
  `timings.jsonl`, never the console tail (Watching 5).
- **Where we are**: the chip's four items are FIXED (51 message · 52 both halves
  ...
  helper's RELOCATION step. Members census (fixed vs still-degrading-loudly) is
  in DEFERRED § 51(c). New items filed this arc: 53–56.
- **Corpus gates**: every completed A/B so far = ZERO semantic diffs (all
  non-identical cases READ: gensym/meta drift + symmetric caps only). The
  CUMULATIVE full-corpus A/B for [else]+def+let (base `6ffc04ef`, 161 files) is
  **IN FLIGHT** at session-scratchpad `corpus-final.txt` / `corpus-out ⁠—
  corpus-final/`; READ it before trusting the arc closed (corpus-ab BUFFERS —
  no output until done; progress = file count in the out dir).
- **NEXT: the SIBLING-LET MERGE (6th fix of the family; owner-chosen).** The
  reproducer, mechanism, and the DESIGN FORK (stx-carrying merge vs deep
  multi-source pool relocation) are in DEFERRED § 51(c) and pinned as a KNOWN
  LIMIT in `test-rel-t1-pol.rkt` ("KNOWN LIMIT: a SIBLING-LET chain") — invert
  that pin on fix, never delete. This is a CO-DESIGN-sized step, not a
  mechanical helper application: `merge-toplevel-sibling-lets` (macros.rkt)
  fuses TWO source stxs into one datum, so single-source alignment structurally
  cannot recover it.
- **Behind it**: 54 · 55 (reader origin marker — now load-bearing under FOUR
  ...
  see DEFERRED § 51(c) relocation notes).
