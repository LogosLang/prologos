...
- **Suite**: **9889 / 483 files / 0 failures** (`all_pass: true`, 108.1s — no
  measurable cost from the helper on the [else] hot arm).
...
- **NEXT**: the one spelling still degrading is UNPARENTHESIZED `def r := rel …`
  with parenless clauses (the DEF arm: `def-rhs-stx` is #f for a spliced
  multi-line RHS, so it falls to the whole-form stamp — measured). Decide whether
  to extend the helper there; the arm carries `prologos-defrhs-command` /
  Q_C semantics, so it is a co-design question, not a mechanical swap. Then 54,
  55 (real diff / reader origin marker), 56, `expr-narrow`, 51(b), and the
  subterm-granularity gate.
- **Open/blockers**: none.
