...
| 2F | **Algebraic Foundation** — endomorphism ring decomposition as data | 🔄 | [Design](2026-03-28_SRE_TRACK2F_DESIGN.md) | — | Kind-variance table, unified duality variances, table-driven dispatch. [Audit](2026-03-28_SRE_TRACK2F_AUDIT.md). [Module Theory](../research/2026-03-28_MODULE_THEORY_LATTICES.md). Pre-0 benchmarks complete. **FOUNDATIONAL** — precedes Tracks 3-7. |
...
| 2H | **Type Lattice Redesign** — union join, subtype-aware meet, per-relation properties | ⬜ | — | — | From Track 2G finding: type lattice NOT distributive under either ordering with current ops. Redesign: subtype join → union types (`Int \| String`), subtype meet → subtype-aware GLB (`Int ⊓ Nat = Nat`). Per-relation properties via Pocket Universe (one cell per domain×relation). Makes type.subtype Heyting → pseudo-complement error reporting. **PREREQUISITE for PPN Track 4** (elaboration on network needs well-structured type lattice). Session lattice may need analogous work. Scope: type-lattice.rkt + unify.rkt + elaborator union handling. Background research needed on union type inference + subtype-aware meet dependencies. |
