...
- [Adhesive Categories and Parse Trees](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) — parse trees are presheaf objects → adhesive. Full DPO toolkit. CALM-adhesive connection.
...
| 2C | Cell References in Expressions — elaborator creates cell-ref not expr-meta | ⬜ | — | — | **ENABLES zonk elimination** (~1300 lines). Deferred from PM Track 10B Phase B2. Expressions reference cells directly; downstream code reads cells, not walks trees. **IS PPN Track 4** (Elaboration as Attribute Evaluation). Also: TMS-aware infra cells + unified type inference/trait resolution under propagator network (from DEFERRED.md, relabeled 2026-03-30). V(2) spec injection (cross-form attribute flow) is Track 4 scope (from PPN Track 2B). |
...
| 2D | **Rewrite Relation** — 4th SRE relation, DPO spans on network | ✅ | [Design](2026-04-03_SRE_TRACK2D_DESIGN.md) | [PIR](2026-04-03_SRE_TRACK2D_PIR.md) | 13/13 rules on SRE (100%). DPO spans, pattern-desc, child-pattern-split, PUnify holes, fold/tree/PU combinators, propagator factory, critical pair analysis (0 pairs, strongly confluent). **Adhesive category confirmed**: parse trees are presheaf objects → full DPO toolkit. [Research note](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md). CALM-adhesive connection identified. |
...
| 2F | **Algebraic Foundation** — endomorphism ring decomposition as data | ✅ | [Design](2026-03-28_SRE_TRACK2F_DESIGN.md) | — (PIR outstanding) | 8/8 phases complete. Kind-variance table, unified duality variances, table-driven dispatch, legacy field removal. 383/383 GREEN, zero behavioral change. PIR not written — tracked as gap. |
| 2G | **Algebraic Domain Awareness** — property-based, not class hierarchy | ✅ | [Design](2026-03-30_SRE_TRACK2G_DESIGN.md) | [PIR](2026-03-30_SRE_TRACK2G_PIR.md) | Property cells, domain registry, meet ops (type+session), ring action, inference with witnesses, implication rules, diagnostic reporting, property-gated behavior. **FINDING**: type lattice NOT distributive under equality merge — type lattice redesign needed for Heyting. **NOTE for future SRE tracks**: `sre-domain` now has 11 positional fields — refactor to keyword arguments when next modifying the struct (L4 from PIR). |
| 2H | **Type Lattice Redesign** — full quantale: union join, tensor, Heyting | ✅ | [Design](2026-04-02_SRE_TRACK2H_DESIGN.md) | [PIR](2026-04-03_SRE_TRACK2H_PIR.md) | Subtype join → union types with absorption. Complete meet (11 ctors, distributes over unions). Tensor (`type-tensor-core` propagator fire fn + scaffolding). Per-relation property declarations. Pseudo-complement scaffolding. V4 distributivity 0/512 (was 412). **Heyting achieved** (ground sublattice). whnf fast-path 625× atoms. 26 commits. **PREREQUISITE MET for PPN Track 4.** |
| 2I | **SD∨ / SD∧ Algebraic-Property Checks** — extend Track 2G registry with semidistributivity | 🔄 | [Design (light)](2026-04-30_SRE_TRACK2I_SD_CHECKS_DESIGN.md) | — | Extends `sre-core.rkt` property infrastructure with SD checks. Implication `distributive ⇒ SD` for Heyting domains; empirical sweep for non-distributive (type×equality, session×equality). Acceptance file deviation justified (too small). Source: [research note §5.4](../research/2026-04-30_LATTICE_VARIETY_AND_CANONICAL_FORM_FOR_SRE.md). |
| 3 | Trait Resolution-on-SRE — impl lookup via structural matching | ⬜ | — | — | Needs Track 1 ✅ (subtyping). Subsumes PM 8E resolution state. May benefit from Track 2H (subtype lattice properties inform resolution strategy). **Adhesive DPO**: coherence checking IS critical pair analysis on impl patterns. See [Adhesive Categories research](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) §6. |
| 4 | Session Types-on-SRE — duality via involution relation | ⬜ | — | — | Needs Track 1 ✅ (duality). Choice/Offer branch duality. `dual` retirement. **Adhesive DPO**: duality is involutive DPO rewriting. Order-independent, all dual steps parallelizable. See [Adhesive Categories research](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) §6. |
| 5 | Pattern Compilation-on-SRE — scrutinee decomposition + NF-Narrowing | ⬜ | — | — | GADT path via unified narrowing. [**Grounding note** (2026-06-29)](2026-06-29_SRE_TRACK5_DEPENDENT_MATCH_GADT_NOTE.md): verified machinery — Σ (= existentials) + `Eq`/`refl`/`J` already present; **the gap is index-refining dependent `match`** (`check-reduce` is non-dependent today, `typing-core.rkt:2522`); formation nearly free, dependent-match is Phase 2. Independent of PM tracks. **Adhesive DPO**: pattern compilation is DPO rewriting on match trees. Compilation order doesn't affect decision tree (adhesive confluence). See [Adhesive Categories research](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) §6. |
| 6 | Reduction-on-SRE — typed graph rewriting engine | ⬜ | — | — | IS PM Track 9. Track Series (5a-5e). E-graphs, tropical semirings. **Adhesive DPO**: β/δ/ι-reduction as DPO spans. E-graphs are adhesive (Arsac 2025). Equality saturation inherits full toolkit. See [Adhesive Categories research](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) §6. |
| 7 | Module Loading-on-SRE — exports/imports as structural matching | ⬜ | — | — | Overlaps PM Track 10. Convergence point. **Adhesive DPO**: name resolution as DPO rewriting. Independent resolutions parallelize. See [Adhesive Categories research](../research/2026-04-03_ADHESIVE_CATEGORIES_PARSE_TREES.md) §6. |
