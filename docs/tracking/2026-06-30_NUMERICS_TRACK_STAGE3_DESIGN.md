...
| **N5** | Refinement substrate (function-level) — **mini-design §9c** (@d carrier = NOMINAL Sign-backed; 8-elem Sign + transfer + decomposed subsumption; migrate 5 refined types) | N0 ✅ (gate cleared) | 🔄 **N5a ✅** (`d73335d7`); N5b–g pending |
...
- **Sequencing** (N0 fixes, in order): **✅ N5a (`d73335d7`)** evacuate the refined `Eq`/`Ord` instances → `prologos::data::refined-instances` (non-prelude), abstract-domains refined-free (§8a fix 3 — prelude-wide) → **N5b** replace the 3 Sign copies + add transfer/Galois impls → **N5c/N5d** decomposed subsumption + arith transfer → **N5e** migrate the 5 types (delete wrappers, re-back Sign) → auto-narrowing → **N5f** fix the classifier (§8a fix 2) → retract line 166 [done §8a].
