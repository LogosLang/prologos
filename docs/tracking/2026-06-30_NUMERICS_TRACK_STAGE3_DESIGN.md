...
| **N6** | Ergonomics capstone — **mini-design §9d** (owner usage-feedback decision set, 2026-07-01): **N6a warnings ✅** (`548502a0` accumulation fix + `a5bb455a` values-only/normalize/dedupe; suite 8486) · N6b literal scheme (`pNN`, Posit32 default, `~` removal) ⬜ · N6c display (sigil-free posits, Rat→fractions) ⬜ · N6d `to-X` family + Float trait instances ⬜ · N6e first-class ops ⬜ · N6f reconciliation (vision/roadmap/audit/book/From→TryFrom/N3g) ⬜ | N3–N5, Q11 | 🔄 N6a ✅; N6b next |
...
**D-N6.5 — `to-X` conversion family.** Source-generic arity-1: `to-float`(=F64)/`to-float32`/`to-float64`, `to-posit`(=P32)/`to-posit8/16/64`, `to-rat`, `to-int`. Classify operand numeric type, route through existing machinery (N3e generics, Q11 `From`, width prims). Total across Posit↔Float (posit-standard NaR↔NaN, Q11); **`to-rat`/`to-int` ERROR on NaN/±Inf/NaR** with a clear message (`TryFrom` remains the Option-returning path) — ergonomic directness over forced matching. Born first-class (D-N6.6).
...
**Sub-phases + gates:** **N6a ✅ (2026-07-02, `548502a0` + `a5bb455a`)** — the `n6a-warning-bugs-diagnosis` workflow (4 facets + adjudicator, H1–H7) root-caused BOTH owner complaints: the "spurious Rat→Float64" warning was **not a real emission** (H1 — fresh exact arithmetic never warns on any path; the owner transcript reproduced exactly as stale accumulation), and the accumulation was **H7′ persistent-facet re-harvest** (the per-command collection propagator folded over the WHOLE persistent attr-map, harvesting all prior commands' `:warnings` facets; facet values persist by design). Fix: retire the whole-map collection propagator → scoped post-quiescence read (current-command expr-tree positions only; also fixed a fire-once harvest lag) + bridge-on-success-only (double-emission guard) + values-only policy (bare literal operands never warn; `expr-nat-val` is the Nat literal node, not suc chains) + exact-side→join text normalization on the detector + display-point dedupe (scaffolding → §12 re-home). The §12 warning-cell re-home confirmed SEPARABLE (resets were never the defect). `tests/test-warning-accumulation.rkt` + literal→silence/value-twin migrations in 3 test files. → **N6b** literal scheme (D-N6.1/2 input side; migration; 3-level WS validation per the behavior change — 6+ test files expected to move) → **N6c** display (D-N6.2 display side + D-N6.3; round-trip property tests updated) → **N6d** `to-X` + Float instances (D-N6.5/7) → **N6e** first-class ops (D-N6.6; mini-design first) → **N6f** reconciliation (D-N6.8). Each: targeted tests → full suite → commit → tracker/dailies per the 5-step checklist. N6b/N6c are behavior changes with broad test-assert fallout — expect the largest migrations.
