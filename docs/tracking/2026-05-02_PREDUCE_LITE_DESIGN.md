...
| 13 | Logic-engine value-tokens held opaque: `expr-cell-id`, `expr-cell-id-type`, `expr-prop-id-type`, `expr-uf-type`, `expr-atms-type`, `expr-assumption-id-type`, `expr-assumption-id-val`, `expr-table-store-type`, `expr-solver-type`, `expr-goal-type`, `expr-derivation-type`, `expr-schema-type`, `expr-answer-type`, `expr-relation-type`, `expr-net-type`. Surface OPS (clause/defr/fact-block forms, atms-amb/atms-assume/etc.) deferred to Phase 13b — these have logic-engine semantics not aligned with PReduce-lite's value-reduction model. | ✅ (values) / ⏭️ (ops to 13b) | |
| 14 | Tail edges: `expr-Open` opaque. Other tail nodes (`broadcast-get`, `cut`, `explain`, `explain-with`, `all-different`, `from-int`, `from-nat`, `panic`) deferred to Phase 14b — most are foreign-fn-style ops or logic-engine forms that fall outside PReduce-lite's MVP scope. | ✅ (values) / ⏭️ (ops to 14b) | |
