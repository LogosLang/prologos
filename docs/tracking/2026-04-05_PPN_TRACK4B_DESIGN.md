...
| 6 | Meta solution bridging (ephemeral → main network) | ✅ | commit `a8b597de`. **Architectural shift**: eliminated ephemeral PU. Typing runs on MAIN network. Meta-bridge propagators + output cell replace scan. Removed: ephemeral PU, constraint-metas collection, scan bridge. Scaffolding: solve-meta! calls (Phase 9 retires). |
| 7 | Warning attribute propagators (S2: accumulation + reporting) | ✅ | commit `cc4bad36`. :warnings merge = append. Usage-validation + warning-collection propagators (S2, P2). Warning output cell. All 5 attribute domains active. |
| 8 | ATMS integration for union type checking (S(-1): retraction) | ⏸️ | **Blocked on BSP-LE Track 1.5** (Cell-Based TMS). Current TMS uses parameter-based speculation stack (off-network). Principled architecture: worldview as cell value (on-network). Option D (concurrent branching) requires cell-based TMS. [Design note](../research/2026-04-06_CELL_BASED_TMS_DESIGN_NOTE.md). |
