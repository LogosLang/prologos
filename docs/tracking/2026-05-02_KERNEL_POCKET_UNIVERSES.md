...
| **3** | Low-PNet IR — write-decl mode tag | `racket/prologos/low-pnet-ir.rkt`, `racket/prologos/tests/test-low-pnet-ir.rkt` | ~30 LOC Racket | Add a write-mode tag to `write-decl` (`:merge` default for backward compat; `:reset` for non-monotone). Update `parse-low-pnet`, `pp-low-pnet`, `validate-low-pnet`. Bump version V11 → V12. **NO new node kinds.** | Round-trip pp ↔ parse for write-decl with both modes; validator accepts both; `:merge` is default when mode tag absent (backward-compat). **All ~22 existing IR tests pass unchanged.** |
...
| **8** | 3 | Low-PNet IR `write-decl` mode tag (`:merge` default; `:reset`); parse / pp / validate; bump V11→V12 | `racket/prologos/low-pnet-ir.rkt`, `tests/test-low-pnet-ir.rkt` | All ~22 IR tests pass; round-trip works |
