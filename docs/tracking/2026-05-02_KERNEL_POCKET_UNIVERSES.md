...
**Category B — Retired by THIS track (Phase 6 deletion)** — all DELETED 2026-05-02 in Phase 6 Day 13 (3 commits).

| Artifact | Where | Status | Retired by |
|---|---|---|---|
| `iter-block-decl` IR node | `racket/prologos/low-pnet-ir.rkt` (provide, struct, parse, pp, validate V11) | **DELETED** | Phase 6 Day 13 commit (2/3): `sh/kernel-pu: Phase 6 Day 13 (2/3) — retire iter-block-decl IR node`. Provide, struct, parse branch, pp branch, validate V11 all removed; downstream consumers (`low-pnet-to-prop-network.rkt` materialization gate, test rejection) updated. Programs containing the node now fail at parse-low-pnet with "unknown decl head". |
| Unused `iter-blocks` builder field | `racket/prologos/ast-to-low-pnet.rkt` (builder struct, ~line 95) | **DELETED** | Phase 6 Day 13 commit (1/3): `sh/kernel-pu: Phase 6 Day 13 (1/3) — retire vestigial iter-blocks builder field`. Field + auto-mutable accessor + initializer in make-builder all removed; tombstone comment cites the substrate-iteration-pattern replacement. |
| Sprint G `@main`-loop emission paths | `racket/prologos/low-pnet-to-llvm.rkt` (Sprint G additions, if any landed) | **NOT LANDED — confirmed dead-on-arrival** | Phase 6 Day 13 inspection: `grep -i 'iter-block\\|iteration\\|Sprint G\\|@main.*loop' low-pnet-to-llvm.rkt` returns zero matches. The `@main`-loop emission path was never written; Sprint G's lowering rewrite ended at the IR-node stage. No deletion needed. |
...
| `run-stratified-resolution!` (imperative variant) | `racket/prologos/metavar-store.rkt:2079` | **DELETED** | Phase 6 Day 13 commit (3/3): `sh/kernel-pu: Phase 6 Day 13 (3/3) — retire run-stratified-resolution! + current-in-stratified-resolution?`. 36-line function body removed; tombstone comment cites Track 8 A5 supersession + zero-caller verification. |
| `current-in-stratified-resolution?` parameter | `racket/prologos/metavar-store.rkt:2081` | **DELETED** | Same Phase 6 Day 13 commit (3/3). With its sole writer (`run-stratified-resolution!`) gone, the parameter always returned #f. `solve-meta!` simplified to drop the guard; re-entrancy now provided structurally by `run-stratified-resolution-pure`'s eq? identity termination. Provide + lint baseline updated. |
...
| **10** | 4b | `low-pnet-to-prop-network` adapter (~285 LOC) + `tools/round-trip-acceptance.rkt` harness + CI integration. Walks Low-PNet IR; allocates cells via `net-new-cell`; installs propagators via `net-add-propagator` with kernel-* fire-fns mapped 1-to-1 from the Zig kernel's `fire_against_snapshot` switch; dispatches write-decl mode tag (`'merge` → `net-cell-write`, `'reset` → `net-cell-reset`); normalizes bool init values to 0/1 to match LLVM-pipeline parity. | `racket/prologos/low-pnet-to-prop-network.rkt` (new), `racket/prologos/tests/test-low-pnet-to-prop-network.rkt` (new), `tools/round-trip-acceptance.rkt` (new), `.github/workflows/network-lower.yml` | **37/37 acceptance examples** (n0 through n7-showcase) round-trip through propagator.rkt and match the binary's exit code semantics (mod-256 normalization handles fib20=6765 vs OS exit 109). 19 unit tests pass. NO LLVM and NO native binary in the loop. |
| **11** | 5a | LLVM emission: write-decl mode dispatch (`'merge` → `@prologos_cell_write`, `'reset` → `@prologos_cell_reset`); conditional `cell_reset` declaration (only when used); opt-in scope-API declarations gated on `(meta-decl uses-scope-apis #t)` for Day 12 NAF migration. | `racket/prologos/low-pnet-to-llvm.rkt`, `tests/test-low-pnet-to-llvm.rkt` | 6 new emission tests pass (+12 baseline = 18). End-to-end: a synthetic IR with `(write-decl 0 73 0 reset)` lowers, links, and runs to exit 73 — `cell_reset` symbol resolved against `runtime/prologos-runtime.zig:336`. **37/37 round-trip-acceptance examples still pass; 173 unit tests across 9 files pass.** |
| **12** | 5b | Category-C migrations: (i) `propagator.rkt` — Racket-side mirror of kernel scope APIs (`scope-enter` / `scope-run` / `scope-read` / `scope-exit` + `scope-result`), thin wrappers over `fork-prop-network` + `run-to-quiescence` + `net-cell-read` that name the per-invocation isolation pattern. `scope-run` returns `RunResult` (`'halt` / `'fuel-exhausted` / `'trap`). (ii) `process-naf-request` re-expressed as a verbatim translation of § 5.9's worked example: per-NAF-aid `scope-enter` (charges parent_fuel_charge=10) → `cell_reset` clears NAF-pending on the scope → install inner goal → `scope-run` (per-scope fuel) → `scope-read` extracts provability → `scope-exit`. (iii) `run-stratified-resolution-pure` migrated from a hard-coded inline S(-1)/S0/S1/S2 pipeline to a data-driven walk over `resolution-strata` (a list of named handler procedures). Adding/reordering strata is now a list edit; the next step folds each handler into a request-cell + handler-propagator subscribed to it (full BSP-driven dispatch). | `racket/prologos/propagator.rkt` (+~110 LOC: scope APIs + provides), `racket/prologos/relations.rkt` (process-naf-request refactor), `racket/prologos/metavar-store.rkt` (run-stratified-resolution-pure refactor), `racket/prologos/tests/test-scope-apis.rkt` (new, 11 tests) | **NAF isolation gate met**: 11/11 scope-API tests pass, including 2 explicit gates ("divergent inner scope leaves parent fuel intact" + "parent stays usable after divergent scope"). Parent fuel debited only by `parent_fuel_charge` (10); scope's divergent volley exhausts to 0 in 100 fuel; parent runs normally afterward. **220/220 regression tests pass** (relational-e2e, relations-runtime, stratified-eval, stratify, all wf-engine tests, propagator, propagator-solver, trait-tycon, trait-narrowing). **37/37 round-trip-acceptance examples still pass** (no IR/emission impact). NAF refactor preserves observable semantics across all NAF-bearing tests. |
| **13** | 6 | Category-B retirements completed (3 commits with tombstone comments): (1/3) **vestigial `iter-blocks` builder field** in `ast-to-low-pnet.rkt` removed (Sprint G accumulator that was never appended-to). (2/3) **`iter-block-decl` IR node** removed from `low-pnet-ir.rkt` (provide, struct, parse branch, pp branch, validator V11 — all gone); downstream consumers updated (`low-pnet-to-prop-network.rkt` materialization gate retired, test rewritten to expect parse-error). (3/3) **`run-stratified-resolution!` + `current-in-stratified-resolution?`** removed from `metavar-store.rkt`; `solve-meta!` simplified (re-entrancy now structural via `run-stratified-resolution-pure`'s eq? identity termination). Sprint-G `@main`-loop emission paths in `low-pnet-to-llvm.rkt` were inspected and confirmed never-landed (zero call sites). Sprint-G depth-alignment + bridge-cell helpers were already exempted from this row at Day 9 (load-bearing under Variant B). | `racket/prologos/ast-to-low-pnet.rkt`, `racket/prologos/low-pnet-ir.rkt`, `racket/prologos/low-pnet-to-prop-network.rkt`, `racket/prologos/metavar-store.rkt`, `racket/prologos/tests/test-low-pnet-to-prop-network.rkt`, `racket/prologos/tools/parameter-lint-baseline.txt` | **§ 9.1 grep-check criterion met**: all remaining `iter-block-decl`, `run-stratified-resolution!`, `current-in-stratified-resolution?` hits are retirement-context comments or historical refs. **142/142 broad regression tests pass** (relational-e2e, relations-runtime, stratified-eval, stratify, wf-engine, trait-tycon, trait-narrowing, constraint-postponement, scope-apis). **37/37 round-trip-acceptance examples still pass**. **106/106 IR + lowering tests pass** (test-low-pnet-ir, test-low-pnet-to-prop-network, test-ast-to-low-pnet, test-low-pnet-to-llvm). |
