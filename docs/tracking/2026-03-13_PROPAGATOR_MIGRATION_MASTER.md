...
**Design input from the GitHub #78 repair (2026-07-27)** — the registry READ PATH: retiring the parameter fallback. The registry half-migration (Track 7 moved registry *contents* to cells; the parameters were never retired) has now produced silent defects **in both directions**: the `.pnet` cache-hit restore wrote parameters only while all 24 readers are cell-primary (issue #78 — stuck terms, wrong answers, durable cache poisoning), and the pure resolution bridges capture registry cell-ids while they are still `#f` and so read an empty impl registry in every process (`resolution.rkt:460-462`, `:552-555`). Neither is caught by the suite. The #78 repair deliberately took the dual-write (Option A) and **deferred the structural cure here**, because retiring the fallback today is *actively dangerous*: `init-macros-cells!`'s un-guarded re-seed after preparse is currently the accidental healing that masks #78's `process-file` shape, so removing it before the replacement invariant exists WIDENS the defect. The note also records the load-bearing constraint that "cells as single source of truth" must not mean "read the global cell everywhere" — `load-module` parameterizes the registries (so the parameter carries **module-scoped accumulation**, which `serialize-module-state` correctly relies on) while cell writes escape, making this slice the read-side face of the **submodule-scope** work above rather than an independent one. Full grounded pickup facts (file:line @ `3f828bc9`): [`2026-07-27_PM_TRACK12_REGISTRY_READ_PATH_NOTE.md`](2026-07-27_PM_TRACK12_REGISTRY_READ_PATH_NOTE.md). Origin: PM Track 10 post-PIR defect repair [`2026-07-27_PM_T10_PNET_REGISTRY_RESTORE_DEFECT_DESIGN.md`](2026-07-27_PM_T10_PNET_REGISTRY_RESTORE_DEFECT_DESIGN.md) §5.1.
