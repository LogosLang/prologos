...
| 6+7 | Propagator-native solver (merged: clause matching + goal dispatch) | 🔄 | 6a: `029b8a5d` make-branch-pu worldview. 7a+7b+6c: `37d891e8` skeleton. Tests+fix: `6d1dfd54`. 6d: `ed10ebef` multi-clause PU branching. Core loop functional (facts, single-clause, multi-clause with PU isolation). 10 tests. Remaining: 6e (answer accumulator), 6f (typing-propagators), 7c (NAF+guard), integration parity. |
