...
6. *Self-Critique (D.4)*: Challenge against all 10 principles. Also
   ask: does this design match what the codebase ACTUALLY does, or
   does it design against a vision of what we WANT it to do?
...
- *Codebase reality check in self-critique*: The self-critique round
  (D.4) must ask: "for each phase in this design, can I point to the
  specific lines of code it changes?" If the answer is "approximately"
  or "I think so," the design is speculative. Run the greps. Check the
  call counts. Verify the function signatures match what the design
  assumes. PM 8F's D.1 assumed ~225 zonk sites; a grep found 31.
  SRE Track 2 assumed 158 typing-core cases; an audit found 0.
  Speculation that isn't grounded in code measurements will be wrong.
