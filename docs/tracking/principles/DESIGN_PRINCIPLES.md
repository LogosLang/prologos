- [Core Values](#org7b72214)
  - [Correctness Through Types](#orgeae7b7a)
  - [Simplicity of Foundation](#org60fb672)
  - [Progressive Disclosure](#orgedaafef)
  - [Pragmatism with Rigor](#org6d77db3)
- [Decomplection](#org55dffad)
  - [Collections and Backends](#org315ba19)
  - [Schema / Defn and Trait Bundles](#orgbf914cf)
  - [No Trait Hierarchies &#x2014; Bundles Only](#org2fb37b2)
  - [Session / Defproc for Protocol Types](#org29d1e6c)
  - [Data vs. Control](#orgc1387e9)
  - [Compile-Time vs. Runtime](#org15715b4)
- [Layered Architecture](#orgfae72a4)
- [First-Class by Default](#org9d89e1f)
  - [The Session Types Lesson](#org6d963d6)
  - [Application](#org5c1b661)
  - [Relationship to Other Principles](#orgd6180d7)
- [The Most Generalizable Interface](#org2c4bc64)
- [Homoiconicity as Invariant](#orgc6e41bf)
- [AI-First Design](#org222a01d)
- [Spec as Living Interface](#org6e26868)
  - [Spec as Living Interface](#org5e1f095)
  - [Open Metadata, Closed Semantics](#org2a7f6d4)
  - [Holes are Conversations](#org95e410b)
  - [Keyword Symmetry](#orgc55dc3f)
  - [Pi Types as Implementation Detail](#org928aa32)
- [Correct by Construction](#orga1ae14e)
  - [Relationship to Other Principles](#orgdb0b25b)
- [Open Extension, Closed Verification](#org78689c0)
- [Propagator-First Infrastructure](#org1738fad)
  - [The Composition Argument](#orgc97fcff)
  - [When Not To Use Propagators](#org216ea1c)
  - [Relationship to First-Class by Default](#orgbc35198)
  - [Application](#orgb9a6bcf)
- [Data Orientation](#orgb55756f)
  - [The Core Pattern](#orge30083a)
  - [Relationship to Other Principles](#orgebdb777)
  - [The Semi-Naive Evaluation Connection](#org66030b6)
  - [The Free Monad Analogy](#org16d6318)
  - [Design Invariant: Propagator Statelessness](#org9746d48)
  - [Application](#org2e987ab)
...
2.  **No superclass brittleness**: Adding/removing a trait from a bundle never breaks downstream code &#x2014; it only changes the expansion
...
Session types in Prologos were designed as protocol specifications. Because they were made first-class types (not a special-purpose annotation system), they could appear anywhere types appear &#x2014; including as continuation type arguments to other session types. This yielded composable protocol phases: a capability that was not designed but *fell out* of the first-class decision. No amount of upfront analysis would have predicted this; it emerged from the combinatorial interaction of session types with the existing type system.
...
-   **Layered architecture**: each layer's constructs are independently useful *because* they are first-class &#x2014; they compose with constructs from other layers
...
The `??` typed hole syntax initiates a dialogue between the programmer and the type system. The type checker responds with the expected type, available bindings, and suggestions. This is not an error &#x2014; it is the normal mode of interactive development in a dependently-typed language.

`??` is fundamentally different from `_` (type inference hole). `_` says "infer this for me automatically" (for the machine). `??` says "I don't know what goes here &#x2014; help me" (for the human). The former is silent; the latter is a conversation.
...
Dependent types are the *implementation substrate* &#x2014; they give the type system its power. But the surface language speaks in domain terms:
...
This principle is the philosophical foundation for [Propagator-First Infrastructure](#org1738fad): building the LSP server's state management on propagator cells makes incremental consistency a structural property of the network topology, rather than a property maintained by hand-written invalidation code.
...
When building internal infrastructure &#x2014; caches, indices, registries, environment maps &#x2014; default to propagator cells over mutable hash tables. The synergy of composing propagator networks exceeds the sum of their parts: each new propagator-backed structure can participate in cross-domain information flow with every other propagator-backed structure, creating emergent capabilities that isolated mutable stores cannot provide.
...
-   A type index cell (srcloc → type) that depends on a metavariable cell automatically updates when the meta is solved &#x2014; no explicit "zonk the index" pass needed.
-   A definition location cell that depends on module loading automatically reflects newly-loaded modules &#x2014; no explicit "rebuild the index" step.
-   A diagnostic cell that depends on type index cells automatically retracts when the source is re-elaborated &#x2014; incremental re-checking falls out of the network topology.
...
This principle is the infrastructure-facing corollary of [First-Class by Default](#org9d89e1f). First-Class by Default says: reify language constructs as first-class values. Propagator-First Infrastructure says: build internal state as first-class network participants. Both principles produce the same kind of emergent composition &#x2014; the combinatorial interaction of independently-designed components creating capabilities that no single component was designed to provide.
...
The action descriptors form a *free algebra* over the effect signature. The interpreter is the *fold* that evaluates the algebra. This separation yields:

1.  **Inspectability**: the worklist is data &#x2014; it can be logged, counted, visualized in the observatory, validated for confluence
...
4.  **Totality**: the interpreter can enforce fuel limits, detect cycles, and guarantee termination &#x2014; structurally, not by convention
...
-   **Propagator-First Infrastructure**: data (cells) over state (mutable hashes). Propagators are data-oriented by construction &#x2014; they read cells, write cells, and compute pure transformations. Data orientation generalizes this pattern beyond infrastructure to *all* effectful computation.
...
-   **Correct by Construction**: data structures that make wrong states unrepresentable. Action descriptors make *wrong orderings* unrepresentable &#x2014; the interpreter controls ordering, not the producer of descriptors.
...
-   **First-Class by Default**: action descriptors are first-class data &#x2014; storable, passable, inspectable, serializable. Embedded effects are none of these things.
...
This invariant ensures that the propagator network remains the single source of truth. Threshold propagators (`make-threshold-fire-fn`, `make-barrier-fire-fn`) demonstrate the correct pattern: they read cell values on every invocation, compute a predicate, and either run the body or return the network unchanged. The "memory" is in the cells, not the closure. On monotone lattices, this stateless approach is observationally identical to a stateful "fire once" propagator &#x2014; but preserves full observability.
