...
| F1b.2 **SOUNDNESS GROUNDWORK** | (a) Route-soundness repair (slice-opening mini-grounding → the ann-adoption fix; + the inline-cons width-tolerance audit+fix) — **prerequisite of F1b.4** · (b) D23 groundwork: structured meta-source-info at the 4 D19 mint sites (loc=#f named) · deepen the shallow scrub walk (union/record recursion) · route ALL stored-type paths through the uniform scrub (closes the raw-meta LEAK; not yet erroring) | D26 + D23g | the P1 probe matrix FLIPS (`(the Int {:a 1})` errors; `the (Map Keyword Int) {:a 2 :b "x"}` errors); the P7 leak probe stores holes not raw metas; suite green |
