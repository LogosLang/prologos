...
| F1a-s3 | Remaining record dispositions | 🔄 | **Grounding audit `wf_2d535113` done** (corrected scope; several under-counts folded). **B3 ✅ `a5c546c8`** (Record-vs-Record same-shape classify case; `tests/test-record-collections.rkt` prelude-loaded; suite GREEN **8572/450/0**). **S10 ✅ already s2** (audit confirmed `ground-expr?` Record arm at `trait-resolution.rkt:70`; records match no Map-headed instance head → clean error; posture to document). **Remaining**: **S3** (fold/filter/map-vals infer arms + **BOTH** union arms map-get+nil-safe-get + **check-side** arms + whnf wrapper + extract `record-value-union` helper) · **S7** (rich closed-row-miss diagnostic — 3 callers, precise gating) · **.pnet cross-module canary** · **B4** acceptance dynamic-key canaries · explicit non-reachability notes (narrowing / transient-persist / definitely-not-map?) |
