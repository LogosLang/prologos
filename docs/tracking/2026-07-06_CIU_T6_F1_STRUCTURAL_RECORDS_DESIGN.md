...
|---|---|---|---|
| F1b.1 **FIRST GREEN** | Echo deletion (six reduction.rkt merge surfaces) · `test-bound-args-01` INVERTS to keys-ABSENT gate · 4 example touch-ups · explain reserved-key interim guard · solve-one → bare-map-or-`none` (verify current shape first) | D25 | solutions display `'[{:unknown …}]`; keys-absent gate green; suite green |
