...
| F1a-s3 | Remaining record dispositions | 🔄 | **Grounding audit `wf_2d535113` done**. **B3 ✅ `a5c546c8`** (same-shape classify case; prelude WS test; 8572/450/0). **S10 ✅ already s2** (`ground-expr?` arm confirmed; posture to document). **S3+B4 ✅ `83bd416f`** (fold/filter/map-vals Record arms in BOTH checkers — filter=dictionary-degrade, map-vals=LABEL-PRESERVING; both union arms accept Record components, Q5 filter + B4 Keyword gate; `record-value-union`+`union-record-component-vt` helpers; whnf wrappers; acceptance 37/37; suite GREEN **8586/451/0**). **Remaining**: **S7** (rich closed-row-miss diagnostic) · **.pnet cross-module canary** · non-reachability notes (narrowing / transient-persist / definitely-not-map?) |
