...
| F1a-col | **D15 observational literal typing**: literal-extent nodes, all-at-once typing — homogeneous → `(PVec T)`/`(List T)` as today; heterogeneous → closed `'nat` rows. Union = derived α only | 🔄 | **col-1 ✅ `cf034b56`**: `expr-pvec-literal` through the pipeline; `@[1 "a" true] : ⟨Int String Bool⟩`; unification-based homogeneity (`@[none [some 1]]` still `PVec (Option Int)`); positional projection incl. bare-Int (`tr[1].b ⇒ 2 : Int`, the owner-scratch idiom); Tuple→PVec α (union-branch aware) at all 5 entry points; `'nat` key-gate MUST-FIX landed; acceptance **48/48**; suite GREEN **8601/452/0**. **Remaining**: col-2 `'[…]` (`expr-list-literal`, 4 producer hooks); col-3 tuple-op dispositions (nth/update/pop/concat/slice exact-where-static; 11-arm whnf discipline); col-4 remaining canaries. Selection surface = OPEN design conversation (track doc) |
