...
| F1a-col | **D15 observational literal typing** (revises flavor-B-as-union): literal-extent nodes `expr-pvec-literal`/`expr-list-literal`, all-at-once typing — homogeneous → `(PVec T)`/`(List T)` as today; heterogeneous → closed `'nat` rows. Union = derived α only. col-1 `@[…]` + projection + α + `'nat` key-gate fix; col-2 `'[…]` (4 producer hooks); col-3 tuple-op dispositions; col-4 canaries | 🔄 | Grounding `wf_8c01845e` done (seed-keyed classifier forced by the 2nd pvec producer; in-place widening contract-blocked; display `⟨Int String⟩` landed s1 + owner-confirmed; `v[0]` bare-Int accepted). Selection surface (postfix-path/broadcast/result-shape) = OPEN, own design conversation (track doc) — col ships only degenerate literal-index |
