...
| — p1a | row-comparison engine + §12.4 per-op dyn table + 5 helpers delegate + C_Cons arms + record-project dyn hook + negative probes | ⬜ | zero mints; "validated (not deployed)" if a session ends here |
...
### §12.4 Owed per-op dyn dispositions (p1a opening deliverable — the table AS SPEC)

The s2/s3 keyword map-op Record arms match `(? expr-Record?)` UNGUARDED — a dyn row would get CLOSED-row semantics (unsound: keys/vals/fold/filter/map-vals enumerate only the KNOWN fields). p1a opens by writing the per-op dyn table (get/nil-safe-get exact-on-known + fresh-meta-on-miss; assoc/dissoc literal = extend/remove preserving `'dyn`; size : Nat stays; keys stays (List Keyword); vals/fold/filter/map-vals must NOT trust ⋃known — degrade or fresh-meta element, decided per-op against the s3 precedents) and adds tail guards. NO arm ships without its row in the table.
