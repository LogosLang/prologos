...
| 1 | Lexer + structure as propagators (char → structured token tree) | ✅ | [Design D.9](2026-03-26_PPN_TRACK1_DESIGN.md), [Audit](2026-03-26_PPN_TRACK1_STAGE2_AUDIT.md), [PIR](2026-03-26_PPN_TRACK1_PIR.md). **5-cell architecture**: 4 RRB embedded cells + 1 tree M-type cell. 380/380 GREEN. 108 tests. reader.rkt switchover complete. ⚠ **NETWORK REALITY CHECK FAILS — corrected 2026-08-02.** The title says "as propagators" and the parse layer installs **ZERO** of them. Measured at HEAD across all seven parse-layer files (`parse-reader` `parse-lattice` `tree-parser` `surface-rewrite` `form-cells` `parser` `macros`): 0 `net-add-propagator`/`-fire-once`/`-broadcast`/`elab-add`/`-cross-domain`/`register-stratum-handler`. The complete set of network ops in `parse-reader.rkt` is 1 `make-prop-network`, 5 `net-new-cell`, 5 `net-cell-write`, 1 `net-cell-read` — a **cell LEDGER** written by a straight-line `net1→net6` sequence. `build-tree-from-domains` is defined as a fire function and then **called directly**, never installed. What the track genuinely delivered (the 5-cell architecture, the switchover, the tests) stands; the ON-NETWORK claim does not. Per `workflow.md`'s mandatory check — "Which `net-add-propagator` calls were added? If zero: the phase did NOT add propagators" — this is the check failing on a ✅ row. |
...
| 3 | Parser as propagators — ON-NETWORK | ✅ | [Design D.5b+§11+§12](2026-04-01_PPN_TRACK3_DESIGN.md), [PIR](2026-04-02_PPN_TRACK3_PIR.md). All forms ON-NETWORK. Per-form cells. SRE domains + ctor-descs. Dependency-set Pocket Universe. Spec cells. parser.rkt retired from WS dispatch. 383/383 GREEN. ⚠ **SAME CORRECTION AS TRACK 1 (2026-08-02)**: "ON-NETWORK" here means **cells**, not propagators — zero are installed anywhere in the parse layer. The per-form cells are real (`form-cells.rkt`, source-line-keyed); nothing reads or writes them via a propagator. Read both rows as *cells landed, propagator wiring unbuilt*. The propagator wiring is the genuinely-unbuilt work the reader-layer section of the [4D Implementation Draft Note](2026-05-19_PPN_4D_IMPLEMENTATION_DRAFT_NOTE.md) now scopes. |
...
**Implementation Draft Note (2026-05-19)**: [PPN 4D Implementation Draft Note — Findings Carried Forward from 1D/1E Exploration](2026-05-19_PPN_4D_IMPLEMENTATION_DRAFT_NOTE.md) — captures Stage 2 audit findings, Realization B research, architectural exploration (Approaches A/B/C/D/E with adversarial principles evaluation), spike findings, durable bench harness, and a critical install-breaks-resolution diagnostic finding from the deferred PPN 4C Phase 9+ Addendum 1D/1E exploration.

**⭐ §12 THE READER LAYER (added 2026-08-02)**: [same note, §12](2026-05-19_PPN_4D_IMPLEMENTATION_DRAFT_NOTE.md) — a seed for the gap in the A–G phase structure. 4D's core proposition names PARSING as a component attribute, but no phase scopes it. §12 records the measured state (**the parse layer installs ZERO propagators** — see the ⚠ on Tracks 1 and 3 above), the four RRB cells whose merge is a size heuristic rather than a lattice op, the **sparse-attribute** argument that reframes Track 8's per-node-cell deferral (population is per-ATTRIBUTE, and absence already IS bottom), the **absence of any node identity** (eq? does not survive rebuild; srcloc collides — and a sentinel wrapper collides with its own payload BY CONSTRUCTION), the ~8× tree-node-vs-syntax-node ambiguity in the phrase "per-node", NTT's `:embedded` PU + `:diff` as the preferred realization (with the **measured O(N²)** of the compound-cell alternative, which `:component-paths` itself triggers), and **three substrate blockers** for any defaulting-on-absence stratum — one of which (the BSP Tier-1 fast path runs no strata) fails SILENTLY. Short-circuits portions of 4D's eventual Stage 1-2 work; identifies architectural questions Stage 3 must address.
