...
| 2 | Surface normalization as rewriting | ⬜ | Replaces macros.rkt preparse (~3000 lines). **Key insight**: tree rewriting IS structural unification — [research note](../research/2026-03-26_TREE_REWRITING_AS_STRUCTURAL_UNIFICATION.md). defmacro patterns register as SRE rewrite rules. Normalization = SRE quiescence. **From Track 1**: mixfix angle-bracket override (decision 2) deferred here. Operates on Track 1's tree M-type cell via SRE decomposition. |
