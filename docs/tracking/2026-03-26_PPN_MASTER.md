...
| 4B | Attribute evaluation — typing + constraints + mult + warnings | 🔄 | [Design D.1](2026-04-05_PPN_TRACK4B_DESIGN.md). Attribute Record PU with 5 facets. CLP-inspired constraint domain. Multi-path component-indexed firing fix. Global attribute store. Retires infer/check + resolve-traits + checkQ + freeze. |
| 4C | Elaboration structural transforms on-network | ⬜ | DEPENDS ON SRE Track 6. elaborate-top-level → attribute propagators via DPO rewriting. Implicit arg insertion, name resolution, macro expansion as propagators. Also absorbs whnf/nf replacement via SRE 6. |
