...
| 4b | Zonk retirement: fan-in default propagator | 🔄 | **4b-i** ✅ meta-readiness infra (`002f7cc3`). **4b-ii-a** ✅ cell-id fast path, 6 files (`4b8f3876`). **4b-deploy** ✅ leaf rules in production pipeline (`2a8c3871`). **4b-ii-b** ⬜ zonk.rkt deletion — blocked on non-leaf rule deployment (Phase 6+ constraint creation on-network). |
...
| 6 | Trait resolution as constraint propagators + SRE domain | 🔄 | ✅ Constraint lattice (`f7ef8665`). ✅ Effects protocol + delegation infra (`b306a8c0`). ⬜ Non-leaf rule full deployment has 22 edge-case failures — needs per-test investigation (arity, higher-rank, spec ordering). |
