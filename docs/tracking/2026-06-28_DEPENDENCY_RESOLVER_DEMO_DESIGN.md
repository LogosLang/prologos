...
| **P1** | `prologos::core::json` pure-Prologos parser + `data Json` ADT; `parse-json : String -> Result Json` | #1 no JSON | ⬜ **UNBLOCKED** (2026-07-02) | Was ⛔ on Numerics; **Num Track 1 is ✅ COMPLETE** (N1–N6f) so Float + exponent lexing are in. ⚠ Re-probe before building: the `JFloat Float` decision was taken 2026-06-30, *before* Num T1 finished — verify it against what actually shipped rather than inheriting it |
...
| **P3** | **Scoped first slice**: per-relation fact cell + single-shot runtime `:from` load propagator | #5 (headline, sliced) | ⬜ **one open decision, not blocked** | The `:from` load propagator + solver read are INDEPENDENT and can ship. What changed (Rel T1 Aspect D, 2026-07-23): **only the cell's lattice GRANULARITY**. §D3's per-relation `monotone-set` is overturned in favour of a **per-relation compound cell keyed by position** (component-path precision, so a fact write doesn't wake every consumer) — owner ruling **Q_A** at [artifact §12](../research/2026-07-23_FACT_REPRESENTATION_QUERY_OPTIMIZATION.md). Settle Q_A, then build. **NTT model required**; incremental/retraction/off-param = **Rel T2** (§11). ⚠ Real near-term dependency is **P4**, not Rel T2: `:from` must read a source, and L3 file IO is R-lens **unproven** |
