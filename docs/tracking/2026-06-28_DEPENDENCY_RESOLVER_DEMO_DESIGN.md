...
| **P1** | `prologos::core::json` pure-Prologos parser + `Json` type; `parse-json : String -> Result Json` | #1 no JSON | ⬜ | nesting + scalars + error positions |
...
- **D1 — JSON value type (P1). LOCKED.** Pure-Prologos parser. ADT `Json := JNull | JBool Bool | JNum Rat | JStr String | JArr [List Json] | JObj [Map String Json]`. **Numbers parsed faithfully into `Rat`** (not strings) — integers are the integral subset of `Rat`. Schema coercion (P2b) narrows `Rat → Nat/Int`. *S-lens caveats*: recursive `data` precedent = `List` (`data/list.prologos:12`); **no SRE ctor-desc registration needed** — `Json` is a plain recursive sum, generic type-checker decomposition suffices. `JNum Rat` **erases int/float identity** (`1.0`==`1`) → a `Nat` coercion needs an *integrality* check, not just range. (If round-tripping ever needs int/float distinction, split `JInt Int | JFloat Rat`.)
...
3. **User `data` ADTs: the type binds but constructors do NOT at WS L3** → blocks the planned `data Json` ADT; **P1 must model JSON with maps/lists/Result**, not a `data` ADT. *Owner: elaborator/PPN.*
...
6. *(minor, found 2026-06-29)* `_` in relational goal-**argument** position is treated as a constant, not an anonymous variable — `(package dep _ lic)` matches nothing; a named throwaway var is required. The vision lists `_` as don't-care, so this is a gap. *Owner: relational.*

---
*P0 CLOSED 2026-06-29 — acceptance core runs 0 errors at L3, gaps filed (§12). Schema-typed `defr` (gap #1) FIXED same day — register + type-check facts, demo now uses it. D.2 critique complete 2026-06-28; P3 re-scoped, RPF track spawned (§11). Next: P1 (pure-Prologos JSON, modeled as maps/lists per finding #3) + P2a (`Validation` applicative); then iterate demo look-and-feel.*
