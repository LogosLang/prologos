...
**Created**: 2026-05-02
**Status**: **DESIGN COMPLETE (2026-06-10)** — Track 0 (founding: 0.1 D.1 + 0.2 D.2 + 0.3 D.3) CLOSED; implementation tracks UNBLOCKED (Track 1 next; queue in the D.1/D.2 locks + the preduce-autonomy handoff). Designed via the 2026-06-10 interactive co-design arc (six panel rounds + owner decisions; see docs/tracking/preduce-autonomy/LEDGER.md).
...
- [PReduce Engineering Inputs from Substrate Research](../research/utm-fl/outputs/preduce-engineering-inputs-from-substrate-research.md) (2026-05-09) — distilled engineering memo derived from the internal note above. Per-track inputs for Tracks 0.1, 0.2, 0.3, 1, 2, 3, 4, 5, 6, 9. **Three owner-approval points flagged** — ALL RESOLVED 2026-06-10: NAC → SM3 D1 (termination-guard NACs dissolve into the ACI law; semantic NACs = extraction-boundary nac-spec); C3.e shared residuation API merge → deferred to Track 4 design; HVM2 → deferred whole with the guard that Track 2's design doc opens with the benchmark-posture decision (D.2 §6). Engineering should consume this memo for immediate Track work; the internal note is the theoretical backing.
...
| 1 | E-class cell substrate — cell type + hashcons + union-find on cells | ✅ | [D.1](2026-06-10_PREDUCE_TRACK01_DESIGN.md) | [PIR](2026-06-10_PREDUCE_TRACK1_PIR.md) | **CLOSED 2026-06-10** (autonomous loop iterations 2-13; ledger): facet substrate (4 reduction facets + :opaque, closed dispatch, scoped bot-filter; `2c48d6b8`/`e350a8c0`) · shape-P delta-notify (~420× flat, precise — over-fire×fire-once lesson; `5ae86ccc`) · PCE/1 single hasher + golden vectors (`98972546`) · e-class product cell + 'eclass-refine relation (`cce768e9`) · hashcons + intern + union-emitter green slice (`ba22d1e1`) · #:after + keep-pending stratum substrate (`01044d05`) · congruence engine + REACTIVE wiring incl. two-level cascades (`8f10fcff`/`a1e6b9ef`) · effect-safety floor (F-A: guards + per-occurrence epoch×path keys + :opaque; `e350a8c0`) · acceptance file (`e1e35675`). NAMED REMAINDERS: SM3 rule-registry universe cell (the bridge unit before Track 2; also the tracked-flake structural fix); D5 probe (observer-hook design); keep-pending behavioral test (at rule dispatch); production :eclass-link writers (Track 2 ingestion); fine-grained Track 1 NTT model (PIR decision); .pnet e-class sections (Track 5). |
| 2 | IN-fragment rule track — interior LADDERED per D.2 §5 (2026-06-10): Phase 0 effect-safety guard (BLOCKING) → arithmetic seed (~12-20 ops) → δ → guarded β | ✅ | [D](2026-06-10_PREDUCE_TRACK2_DESIGN.md) | [PIR](2026-06-10_PREDUCE_TRACK2_PIR.md) | **CLOSED 2026-06-10** (loop iters 17-26): exit criterion HOLDS (guard ✓ guarded β fires ✓ PRN §2 confirmation recorded ✓). All hooks gated default-OFF; the close A/B says the flip criterion is NOT met at current hook coverage (+38% on fresh-heavy small files, neutral on large) — the payoff curve points at Tracks 4/5 as chartered. HVM2 §1 = B+C owner-provisional. | EXIT CRITERION (owner-bound): not done until the guard passes AND guarded β fires AND PRN §2 confirmation recorded. Design doc MUST open with the HVM2 benchmark-posture decision (deferred 2026-06-10 with this guard). |
| 3 | First adhesive-DPO rule kind: ι (dual-Nat-rep pairs + user expr-reduce) | ✅ | [D](2026-06-10_PREDUCE_TRACK3_DESIGN.md) | [PIR](2026-06-10_PREDUCE_TRACK3_PIR.md) | Critical pairs resolve by lattice JOIN (SM4 F4). OPENS with the blocking implicit-NAC verification (D.2 §4) — only then does the pattern-completion-vs-nac-spec choice exist. **CLOSED 2026-06-10** (loop iters 40-42): the blocking NAC verification RESOLVED (no NAC machinery — the compiler is already pattern-complete); ι live on the recursion carriers; THE SERIES VERDICT: recording is wall-clock-FREE but memo value ≤ cost on this substrate — the pre-registered boundary fires; the answer is SH/Zig lowering, with the complete measured dossier (PIR §15). |
| 4 | Cost-guided extraction — tropical-quantale residuation on e-class poset | ✅ | [D](2026-06-10_PREDUCE_TRACK4_DESIGN.md) | [PIR](2026-06-10_PREDUCE_TRACK4_PIR.md) | **CLOSED 2026-06-10** (loop iters 27-32): extraction runs as a propagator FIXPOINT (diamond/cascade proven); the SM6 store lives (zero-allocation hits); residuation = 1B's first consumer. THE HONEST VERDICT: the payoff curve has NOT turned — blanket ingestion's standing overhead (+130ms acceptance) exceeds the memo's recovery on every corpus in hand, while the memo's marginal cost on repeats is ~0 (proven); the named lever is SELECTIVE ingestion + the consult-wiring + Track 5 cross-file reuse (PIR §15). |
| 5 | Persistence — content-addressed e-class storage + `.pnet` round-trip + cross-session loading | ✅ | [D](2026-06-10_PREDUCE_TRACK5_DESIGN.md) · [PIR](2026-06-10_PREDUCE_TRACK5_PIR.md) | — | Merckx et al. 2026 realized on our substrate. Implements D.3's frozen spec: PCE/1 encoder + golden vectors, .pnet/2 migration (one-time cold rebuild), question sidecar + epoch GC, serialize-time projection; per-module e-class sections gated on D5 cache-hit data (the rewrites registry earns its keep first). **CLOSED 2026-06-10** (loop iters 34-39): the .pnet/2 container realized; warm-start exists end-to-end; warm ≈ cold at current coverage (pre-registered); the prn residue substrate finding (DEFERRED). |
