...
| 0.3 | `.pnet` extension + LLVM lowering interface | ✅ | [D.3](2026-06-10_PREDUCE_TRACK0.3_PNET_LOWERING.md) | — | **CLOSED 2026-06-10.** PCE/1 identity package signed off (canonical encoding + sha256; D3 rulings; key-space closed incl. rule-set-digest); .pnet/2 tagged-section container; boundary payload tier 2′ (cell-record frozen = the SH Track 1 joint seam; propagator-record reserved). PoC pinning = named owner action. |
