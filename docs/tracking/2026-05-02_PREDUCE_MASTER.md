...
| 3 | First adhesive-DPO rule kind: ι (dual-Nat-rep pairs + user expr-reduce) | 🔄 | [D (opening)](2026-06-10_PREDUCE_TRACK3_DESIGN.md) | — | Critical pairs resolve by lattice JOIN (SM4 F4). OPENS with the blocking implicit-NAC verification (D.2 §4) — only then does the pattern-completion-vs-nac-spec choice exist. |
