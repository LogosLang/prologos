...
**SECOND OCCURRENCE (2026-07-28) — and it sharpens the workaround.** Same
defect, hit while fixing the Syrup byte-length prefix. The change was in
`syrup-wire.prologos`; I deleted `data/cache/pnet/prologos/ocapn/syrup-wire.pnet`
specifically, twice, and the old code kept running. Verified the edit was in
the file by grep, verified the `.pnet` had been regenerated by mtime, and still
got the pre-edit output. Only `rm -rf data/cache/pnet` fixed it.

That is the transitive-dependency hole stated from the other side: deleting the
CHANGED module's own cache is not sufficient, because its CONSUMERS have
embedded the old body and will happily re-emit it — regenerating the very file
you just deleted, from their stale copy. So the workaround is not "delete the
module's `.pnet`", it is **delete the whole cache tree**. The narrower delete
looks like it should work, which is what makes it cost time.
...
### #50 — A multi-line continuation inside a `match` arm fails at MODULE-IMPORT with a bare "Type mismatch", while the same file processes clean on its own (2026-07-28, real bug, **misleading diagnostic + false all-clear**)

**Symptom.** This arm, written in the style used everywhere else in the file:

```
    | syrup-tagged tag p ->
        str::append "<"
          [str::append [encode [syrup-symbol tag]]
            [str::append [re-encode-payload re-encode p] ">"]]
```

produces, at the point some OTHER module imports this one:

```
imports: Error loading module prologos::ocapn::syrup-wire: Type mismatch
```

No line, no column, no expression, no expected-vs-actual. Putting the same
expression on ONE line fixes it completely:

```
    | syrup-tagged tag p -> str::append "<" [str::append [encode [syrup-symbol tag]] [str::append [re-encode-payload re-encode p] ">"]]
```

**Why it is worse than a layout error.** The same layout is CORRECT at `defn`
body level — `encode-record`, ten lines above in the same file, is exactly this
shape and has always worked:
...
So the working example is right there to copy, and copying it into a `match`
arm breaks. The `|` column apparently changes how a continuation line whose
first token is `[` gets grouped.

**And the obvious check says it is fine.** `racket driver.rkt <the module>`
processes the broken file with **0 errors** and a full clean counter dump. The
failure only appears via `require` from another module. So the natural
bisection move — "run the file directly to see what's wrong with it" — returns
an all-clear, and it is easy to conclude the problem is in the importer.

**Diagnosis that works.** Bisect by deletion against an IMPORTING probe file,
never against the module itself. Beware a trap while doing so: if you delete
the definition your probe `:refer`s, you get *the same* "Type mismatch"
message for the entirely different reason that the name no longer exists —
which invalidates that bisection step silently. Keep the probe referring only
to names that survive every step.

**Rule.** Inside a `match` arm, keep the arm body on one line, or factor it
into a helper. Do not carry `defn`-body layout into an arm. Related to
