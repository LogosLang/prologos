...
> **SUPERSEDED by the Stage-3 design doc** [`2026-06-30_NUMERICS_TRACK_STAGE3_DESIGN.md`](2026-06-30_NUMERICS_TRACK_STAGE3_DESIGN.md) (D.1, 2026-06-30) — the detailed, implementable phases (N0–N6), NTT model, SRE lens, literal/display scheme, Float footprint, migration, and per-phase tests live there. The provisional table below is retained for history.
...
**Cluster 4 resolved (2026-06-30):** Q8 = **option D** (context-typed/polymorphic literal — bare `3.14` exact-at-source, type-from-context, unconstrained→exact Rat) + display polish (exact Rat → terminating decimal `3.14`/`0.3`, else fraction `1/3`); research-informed (fixed-Rat footgun real per Raku; fixed-Posit doesn't deliver decimal exactness; D reconciles + fits Prologos elaboration; A dominated). Q9 = exponent literals exact-at-source under D. Q10 = shortest-round-tripping marked-literal display (`~3.14`/`3.14f`) + the Rat decimal rule. Q11 = Float new rank family (exact auto-absorbs; Posit↔Float explicit-only); `3.14f` = Float64. **The N4 design dialogue is COMPLETE** — next = Stage-3 design doc + implementation. **DEMO P1 unblocks after N1 (exponent lex) + N3 (Float).**
