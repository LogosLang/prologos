...
7. *Surface Syntax* — ~defr~, ~rel~, ~solve~, ~explain~, ~solver~, ~&>~
...
This phase includes the ~schema~ keyword — the relational specification form
that completes the ~spec~/~defn~, ~schema~/~defr~, ~session~/~defproc~ triple.
A ~schema~ declares a named, closed, validated map type used for:
- Typed relational facts (column names + types + positional ordering)
- Validated message shapes in session-typed protocols
- Closed record types (Rust struct–like, with dot-access)

See [[file:principles/RELATIONAL_LANGUAGE_VISION.org][RELATIONAL_LANGUAGE_VISION.org § Schema: The Object Language]]
for the full design.
...
The =-with= forms have three parse shapes, disambiguated by whether the
first argument after the keyword is an identifier (solver name) or a
={...}= map literal (inline overrides):
