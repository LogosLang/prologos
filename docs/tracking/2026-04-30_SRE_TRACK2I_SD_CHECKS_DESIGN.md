...
| 15 | Day's doubling / inflation detection | ✅ | _Code + sweep + Nation report integration in single commit per Phase 11+ in-phase pattern (2026-05-08)._ Algorithm: Option B (Adaricheva-Nation 2017 all-or-nothing pair detection). Added `dd-evidence` 5-field struct + `test-admits-day-doubling/detailed` + wrapper + `dd-classify-side` helper. Atomic wiring: sre-core.rkt provides + props-22 + evidence map + `all-sweep-properties` (now 20) + `sweep-one-property` dispatch + `extract-detailed-fields` (sre-property-sweep + run-phase9-sweep both copies per Phase 11 lesson). 7 new test-cases. Targeted suite GREEN: 196 tests / 4.5s. **Phase 15 finding**: 8/10 (domain × relation × depth) tuples confirm admittance with concrete witnesses (session 100% non-vacuity, type×sub×wider 92%, form/spec ~30-40%). Type×{eq,sub}×ground refute-on-sample is a struct-constructor hash-collision artifact (atomic-pair canonicalization filters out incomparable pairs in both directions); wider samples provide substantive answer. Honest scope: pair-only detection; extended-set search (Tier 4 follow-up) deferred. No forward implication rule (sample-unsound, Phase 12+13 precedent). Adversarial VAG passed; reopened-arc VAG (Phases 11-15 unit) deferred to Track 2I close commit. |
