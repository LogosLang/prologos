...
| 3 | Architecture selection | Auto-detect based on process structure | Multi-channel IO → D; single-channel/REPL → A |
...
| 7 | Data-flow analysis approach | Static AST walk (not incremental propagation) | Sufficient for Phase 0; incremental analysis is a future optimization |
| 8 | ATMS integration scope | =proc-case= branches only (not speculative IO) | Speculative IO is research; =proc-case= is the concrete need |
...
| 10 | File format | =.org= (canonical per project rules) | =.md= is generated artifact |
...
AD-C2 (Position Cells in Runtime) ── requires AD-B, AD-C1 ───────┤
                                                                  │
AD-D1 (Data-Flow Edge Extraction) ── requires AD-A ──────────────┤
...
Critical path: AD-A → AD-B1 → AD-C1+C2 → AD-D3 → AD-E3 → AD-F2
#+end_example
...
;; A vector position: multiple channels' positions (Lamport vector clock)
...
;; An effect descriptor: what effect to perform + at what position
(struct effect-desc
  (kind       ;; symbol: 'open | 'write | 'read | 'close
   channel    ;; symbol: which channel
   position   ;; eff-pos: causal position in the timeline
   payload    ;; any: the data (path for open, value for write, type for read, #f for close)
   )
  #:transparent)

;; Effect accumulator: set of effect descriptors (monotone via set-union)
...
#+begin_example
(effect-desc 'write channel (eff-pos channel depth) str-val)
#+end_example
...
(define (extract-data-flow-edges proc [chan-depth (hasheq)] [var-origins (hasheq)])
  (match proc
    ...
    [(proc-recv chan type cont)
     ...
     ;; The bound variable name comes from the continuation's binding structure
     ;; In practice, we extract it from the cont's free variables or from
     ;; the process elaboration metadata
     (define bound-var (extract-recv-binding cont))
     (define new-origins
       (if bound-var
           (hash-set var-origins bound-var pos)
           var-origins))
...
(define (execute-effects rnet effects)
  (for/fold ([rnet* rnet]
            ...
            ([eff (in-list effects)])
    (match (effect-desc-kind eff)
      ['open
       (define path (effect-desc-payload eff))
       (define chan (effect-desc-channel eff))
       ;; Reuse io-bridge-open-file from io-bridge.rkt (line 131)
       ;; Open file, get port
       (define port (open-input-file path #:mode 'text))
       (values rnet*
               (hash-set results (effect-desc-position eff) port)
               (hash-set open-ports chan port))]

      ['write
       (define val (effect-desc-payload eff))
       (define chan (effect-desc-channel eff))
       (define port (hash-ref open-ports chan))
       (write-string (if (string? val) val (format "~a" val)) port)
       (flush-output port)
       (values rnet*
               (hash-set results (effect-desc-position eff) (void))
               open-ports)]

      ['read
       (define chan (effect-desc-channel eff))
       (define port (hash-ref open-ports chan))
       (define data (port->string port))
       ;; Feed result back into propagator network (msg-in cell)
       (values rnet*
               (hash-set results (effect-desc-position eff) data)
               open-ports)]
...
(define (rt-execute-process proc session-type [fuel 1000000] #:architecture [arch 'auto])
  (case arch
    ...
    [(auto)
     (if (process-has-multi-channel-io? proc)
         (rt-execute-process-d proc session-type fuel)
         (rt-execute-process-a proc session-type fuel))]))
#+end_src

Override via strategy parameter for manual control.
