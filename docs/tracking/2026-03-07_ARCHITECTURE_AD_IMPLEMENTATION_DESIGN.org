...
|-------+-----------+--------+--------+-------|
| AD-A | A0: =proc-recv= binding preservation | COMPLETE | =9dce7dd= | 9 source + 8 test files; 5860 tests pass |
| AD-A | A1: Effect Position Lattice | COMPLETE | =0f20fec= | =effect-position.rkt=: lattice + ordering + session-depth |
| AD-A | A2: Effect Descriptor Type | COMPLETE | =0f20fec= | Sum-type descriptors + effect-set accumulator; 54 new tests |
| AD-B | B1: Single-Channel Bridge | NOT STARTED | | =effect-bridge.rkt= |
| AD-B | B2: Multi-Channel Bridge | NOT STARTED | | Vector clock positions |
| AD-C | C1: Effect Collection Mode | NOT STARTED | | =#:collect-effects?= in =compile-live-process= |
| AD-C | C2: Position Cells in Runtime | NOT STARTED | | Bridge propagators in runtime network |
| AD-D | D1: Data-Flow Edge Extraction | NOT STARTED | | =effect-ordering.rkt= |
| AD-D | D2: Transitive Closure Propagator | NOT STARTED | | Monotone fixed-point computation |
| AD-D | D3: Integration with Collection | NOT STARTED | | Full pipeline wiring |
| AD-E | E1: Effect Linearization | NOT STARTED | | Topological sort |
| AD-E | E2: Effect Executor | NOT STARTED | | =effect-executor.rkt= |
| AD-E | E3: Full D Pipeline | NOT STARTED | | =rt-execute-process-d= |
| AD-F | F1: ATMS Branching | NOT STARTED | | =proc-case= worldview management |
| AD-F | F2: Architecture Selection | NOT STARTED | | Auto-detect A vs D |
| AD-F | F3: Concurrent Hooks | NOT STARTED | | Placeholder for S8b runtime |
