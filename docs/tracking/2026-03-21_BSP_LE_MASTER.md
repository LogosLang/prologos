...
| 2B-A3-static-lint | Static lint for unclassified `make-parameter` calls | Grep-based CI check: classify each `make-parameter` site (module-private / test-harness-registered / documented test-safe / unclassified). Unclassified → CI failure. New `make-parameter` defaults to unclassified until classified. | ⬜ | Addendum scope | **Tactical near-term protection** while PM Track 12 is pending. ~1-2 days. Prevents silent regressions as parameters are added. Obsoletes itself when PM 12 delivers (cells isolate by construction; no need for parameter classification). Simple, compatible with any future migration approach. |
