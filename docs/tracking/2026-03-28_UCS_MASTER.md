...
| 6 | Types-as-Predicates — `?x:Type` domain-constrained logic vars | ⬜ | Spawned from Rel T1 Aspect C (2026-07-21). `?x:Int` = Curry-Howard `Int(x)` = runtime domain-constraint (altitude 2; the static altitude is DONE in Rel T1, refinement `Int@pos` is Track 5). Inherits the C-arc substrate (the `type-pred` value + fused `?x:Int` reader, both readers/langs) + the existing (dead-in-WS) `?var:C1:C2` narrowing → `type-guard` mechanism to bridge/revive. Note: [[file:2026-07-21_UCS_TYPES_AS_PREDICATES_NOTE.md][Types-as-Predicates]]. Key gaps: expr→symbol down-projection, the 5-symbol runtime ceiling (`value-has-type-tag?` stub), the S(-1) non-monotone cap, the domain-annotation reader gap, Int=infinite domain (widening/narrowing). Partly answers Open Question #5. |
